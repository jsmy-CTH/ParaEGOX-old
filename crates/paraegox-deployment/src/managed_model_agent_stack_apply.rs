...
impl ArtifactExternalControllerStateV2 {
    ...
    pub(crate) fn begin_apply(
        ...
        if self.phase != ArtifactExternalControllerPhaseV2::Committed
            || self.records.len() != 1
        {
            return Err(ManagedModelAgentStackApplyControllerError::InvalidPhase);
        }
    ...
    pub(crate) fn finish_apply(
        ...
        if self.phase != ArtifactExternalControllerPhaseV2::Applying
            || self.records.len() != 2
        {
            return Err(ManagedModelAgentStackApplyControllerError::InvalidPhase);
        }
        ...
        let progress = ArtifactExternalDeploymentProgressV1::try_new(
            ...
            self.records.last().map(|record| record.progress.lifecycle_generation),
        )?;

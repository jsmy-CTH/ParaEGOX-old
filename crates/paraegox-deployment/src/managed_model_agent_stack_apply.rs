...
fn decode_lower_hex_nibble(
    value: u8,
) -> Result<u8, ManagedModelAgentStackApplyControllerError> {
    match value {
...
mod tests {
    ...
    fn artifact_external_record_and_receipt_preserve_verified_prefixes() {
        ...
        assert!(receipt_ref.encode().to_ascii_uppercase().parse::<DeploymentReceiptRefV1>().is_err());
        assert!(
            receipt_ref
                .encode()
                .replacen(":1:", ":01:", 1)

...
const EXTERNAL_REQUEST_DIGEST_DOMAIN: &[u8] =
    b"paraegox.deployment.external-request.sha256.v1";
const EXTERNAL_ADMISSION_DIGEST_DOMAIN: &[u8] =
    b"paraegox.deployment.external-admission.sha256.v1";
...
impl ArtifactExternalDeploymentAdmissionV1 {
    pub(crate) fn try_new(
        ...
        let admission_digest =
            raw_sha256(EXTERNAL_ADMISSION_DIGEST_DOMAIN, &canonical_wire[..208]);
...
mod tests {
    ...
    fn artifact_external_request() -> ArtifactExternalDeploymentRequestV1 {
        ...
        let binding = ArtifactExecutionBindingV1::try_new(
            pair.object_ref(),
            receipt,
            paraegox_runtime_contracts::managed_model_agent_stack_plan::artifact_execution_profile_commitment_v1(),
        )
        ...
        ArtifactExternalDeploymentRequestV1::try_new(
            ...
            ArtifactConfigCommitmentV1::try_from_bytes([0x44; 32])
                .expect("config commitment"),
            binding,
        )
        .expect("PXDQ")
    }
    ...
    fn artifact_external_request_and_admission_are_exact_correlated_frames() {
        ...
        assert!(
            ArtifactDeploymentOperationIdV1::try_from_bytes([0; 16]).is_err()
        );
        assert!(
            ArtifactExternalDeploymentAdmissionV1::try_new(
                [0; 32],

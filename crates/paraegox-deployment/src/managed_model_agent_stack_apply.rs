...
mod artifact_external_store {
    ...
    impl ArtifactExternalDeploymentControllerStoreV1 {
        ...
        > {
            ...
            if store.state.request().operation_id() != operation_id {
                return release_locked(store, Err(ArtifactExternalControllerStoreFailureV1::NotFound))
                    .map(|_| unreachable!());
            }
            Ok((binding, store))
        }
    ...
    fn open_directory_at(
        ...
        .map_err(|error| match error {
            ...
            | nix::errno::Errno::ENOTDIR => ArtifactExternalControllerStoreFailureV1::Owner,
            _ => ArtifactExternalControllerStoreFailureV1::Io,
        })?;
    ...
    fn scan_names(
        ...
        .map_err(|_| ArtifactExternalControllerStoreFailureV1::Io)?;
        let mut stream = Dir::from_fd(owned)
            .map_err(|_| ArtifactExternalControllerStoreFailureV1::Io)?;
        ...
        for entry in stream.iter() {
            ...
            if bytes.contains(&0)
                || !names.insert(OsStr::from_bytes(bytes).to_os_string())
            {
                return Err(ArtifactExternalControllerStoreFailureV1::Owner);
            }
        }
    ...
    fn clone_state_root(
        ...
        let leaf = reopen_named_directory(
            &parent,
    ...
    fn run_admit(
        ...
        let state = ArtifactExternalControllerStateV2::admit(request.clone(), instance)
            .map_err(|_| ArtifactExternalControllerStoreFailureV1::Owner)?;
        let bytes = state
            .encode()
            .map_err(|_| ArtifactExternalControllerStoreFailureV1::Owner)?;
    ...
    fn classify_snapshot_rename(
        ...
        let active = read_state(&store.root, SNAPSHOT_NAME);
        let candidate = read_state(&store.root, NEXT_NAME);
        match (active, candidate) {
            (
                ...
                && candidate_identity == next_identity => SnapshotRenameState::Old,
            (Ok((active, active_bytes, active_identity)), Err(_))
                if active == *next
                    && active_bytes.as_ref() == next_bytes
                    && active_identity == next_identity => SnapshotRenameState::New,
    ...
    fn settle_successor(
        ...
        let (reopened, reopened_bytes, reopened_identity) = read_state(&root, SNAPSHOT_NAME)?;
        if reopened != next
            || reopened_bytes != next_bytes
            || reopened_identity != next_identity
        {
            return Err(ArtifactExternalControllerStoreFailureV1::Owner);
        }
    ...
    fn commit_successor(
        ...
        if let Some((existing, bytes, identity)) = read_validated_next(store)? {
            ...
            return settle_successor(
                store, authority, binding, next, bytes, identity, tracker,
            );
        }
        revalidate_current_authority(authority, binding, &store.state_root)?;
        let bytes = next

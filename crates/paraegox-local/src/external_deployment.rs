...
#[cfg(unix)]
const EXTERNAL_DEPLOY_STARTUP_TIMEOUT: Duration = Duration::from_secs(120);
#[cfg(unix)]
const EXTERNAL_DEPLOY_POLL_INTERVAL: Duration = Duration::from_millis(50);
...
/// the read-only Controller facade; deploy starts exactly one hidden resident
/// and returns only after its requested generation is correlated with the
...
fn run_deploy_preflight(command: ArtifactExternalDeployCommandV1) -> ProjectionV1 {
    ...
    if controller_preflight.result().is_err_and(|failure| {
        ...
        return project_controller_query(operation_id, controller_preflight);
    }
    let managed_config = match config::parse_managed_chat_supervisor_config(
        command.config().as_os_str().to_os_string(),
    ) {
        Ok(value) => value,
        Err(error) => return ProjectionV1::error(Some(operation_id), error.into()),
    };
    if authority_config.config_commitment().as_bytes() != &managed_config.config_commitment() {
        return ProjectionV1::error(
            Some(operation_id),
    ...
    drop(bundle);
    let request = match DeveloperArtifactExternalControllerRequestV1::try_new(
        controller_operation_id,
        authority_config.config_commitment(),
        object_ref,
        receipt_ref,
    ) {
        Some(value) => value,
    ...
    match controller_preflight.into_result() {
        Ok(projection) => {
            ...
            return project_controller_state_with_changed(&projection, Some(false));
        }
        Err(
        ...
        Err(failure) => return project_controller_query_failure(operation_id, failure),
    }
...
#[cfg(unix)]
fn wait_for_artifact_external_owner(
    config: &config::LocalManagedChatConfigV1,
    operation_id: ArtifactExternalDeploymentOperationIdInputV1,
    request: DeveloperArtifactExternalControllerRequestV1,
    mut child: lifecycle::ArtifactExternalSupervisorChildV1,
) -> ProjectionV1 {
    let deadline = Instant::now() + EXTERNAL_DEPLOY_STARTUP_TIMEOUT;
    let expected_generation = child.generation().to_owned();
    let mut last_progress = None;
    loop {
        let child_state = match child.poll() {
            Ok(value) => value,
            Err(_) => {
                return verified_request_uncertain_projection(operation_id, &request, None);
            }
        };
        ...
        let child_exit_code = match child_state {
            ...
            ArtifactExternalSupervisorChildStateV1::Running
            | ArtifactExternalSupervisorChildStateV1::Contended => None,
        };
        let lifecycle_observation = lifecycle::run_status(config).ok();
        ...
        let lifecycle_running = generation_accepted
        ...
        if controller_observation_allowed(lifecycle_running, child_exit_code.is_some()) {
            ...
            match invocation.into_result() {
                ...
                Err(failure) => {
                    let mut projection = project_controller_query_failure(operation_id, failure);
                    if generation_accepted {
                        projection.changed = None;
                    }
                    return projection;
                }
            }
        ...
        if let Some(exit_code) = child_exit_code {
            if exit_code == Some(2) && !generation_accepted && last_progress.is_none() {
                return projection_error_with_changed(
                    operation_id,
            ...
            return last_progress.as_ref().map_or_else(
                || {
        ...
        if Instant::now() >= deadline {
            return last_progress.as_ref().map_or_else(
                || verified_request_uncertain_projection(operation_id, &request, None),
                |projection| uncertain_controller_projection(projection, None),
            );
        }
...
#[cfg(unix)]
fn query_external_controller(
    config: &config::LocalManagedChatConfigV1,
    operation_id: ArtifactDeploymentOperationIdV1,
) -> DeveloperArtifactExternalControllerInvocationV1 {
    let mut authority =
        RevalidatingExternalControllerAuthority::from_path(config.source_path().to_path_buf());
    DeveloperArtifactExternalControllerV1::query(&mut authority, operation_id)
}
...
#[cfg(unix)]
fn verified_request_uncertain_projection(
    operation_id: ArtifactExternalDeploymentOperationIdInputV1,
    request: &DeveloperArtifactExternalControllerRequestV1,
    changed: Option<bool>,
) -> ProjectionV1 {
    ProjectionV1 {
        ...
        profile: Some(PROFILE),
        artifact_object_ref: Some(request.object_ref().to_string()),
        materialization_receipt_ref: Some(request.materialization_receipt_ref().to_string()),
        generation: None,
        deployment_revision: None,
        controller_snapshot_sequence: None,
        deployment_receipt_ref: None,
        runtime_apply_request_digest: None,
        runtime_terminal_receipt_digest: None,
        terminal_outcome: Some("uncertain"),
        error: Some(LocalProcessError::ArtifactExternalDeployUncertain),
    }
}

#[cfg(unix)]
fn uncertain_controller_projection(
    projection: &DeveloperArtifactExternalControllerProjectionV1,
    changed: Option<bool>,
) -> ProjectionV1 {
...
fn projection_error_with_changed(
    operation_id: ArtifactExternalDeploymentOperationIdInputV1,
    error: LocalProcessError,
    changed: Option<bool>,
) -> ProjectionV1 {
    let mut projection = ProjectionV1::error(Some(operation_id), error);
...
fn project_controller_query(
    ...
    match invocation.into_result() {
        Ok(projection) => project_controller_state_with_changed(&projection, Some(false)),
        Err(failure) => project_controller_query_failure(operation_id, failure),
    }
}

#[cfg(unix)]
fn project_controller_state_with_changed(
    projection: &DeveloperArtifactExternalControllerProjectionV1,
    changed: Option<bool>,
) -> ProjectionV1 {
    ...
    ProjectionV1::from_controller(projection, state, terminal_outcome, error, changed)
}

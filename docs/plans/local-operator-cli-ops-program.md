...
> 当前 D0 动作：D0a 的 exact CLI/JSON、compiled-in deterministic deployment 与真实 binary system evidence 已在 r363 达到 exact-ref `Validated`；它不依赖 external Artifact，不触发 ADR-0004 A0。ADR-0011 及其 authorization receipt 已 Accepted；A1-internal→D0b Artifact F0 joint candidate 已在 r606 immutable ref `e175f2a75c299f67e89ef1b7308f0263d363148e` 达到 exact-ref `Validated candidate`。A1仍没有独立public/admission状态，r606尚未merge/release；R0 replace/restart继续等待该merge gate，D1 rollback继续等待R0，二者都仍未实现
> 当前 Artifact F0 动作：首个且唯一的 profile 是 `developer-local-echo-prefix-v1`；r606已在同一ref完成bounded non-executable model-data的reproducible build、zero-mutation inspect、immutable materialization、fresh D0b Controller/Runtime ActiveReady与TUI/Agent真实消费，并同步登记六条surface与owner API。Ubuntu run `31555989066` 的joint evidence为A1 15/15、D0b 1/1，完整pytest 607 passed/2 root-only harness skipped，governance、workspace Rust、ProcessDomain与doctest全部PASS；这只是validated merge candidate，不是整个Program、R0/D1、remote能力或main发布完成声明

## 一句话结果

ParaEGOX 当前先交付一条普通开发者能直接验证的本地路线：初始化私有工作区、检查配置、以 compiled-in `deterministic-echo-v1` 经真实 DeploymentController/Runtime 部署到 `ActiveReady`，读取 Inspection，并用独立 TUI 附着该已运行 generation。M4a 先只读取该 generation 的 verified PXMT Receipt；只有 M4b 完成 durable PXEV、失败后可查与 bounded structured logs 后，M5b 才把这些诊断数据汇入同一 TUI。ADR-0011 已 Accepted，`developer-local-echo-prefix-v1` 的build/inspect/materialize与fresh D0b Controller/Runtime/TUI真实消费已经在同一r606 immutable ref通过joint implementation、governance与Ubuntu exact-ref验证；该ref仍是待merge/release候选，不把分支存在冒充main用户发布。下一执行阶段是尚未实现的R0 explicit replace/restart，之后才是D1 forward rollback；再往后仍按G0 gate进入Node enrollment、只传输不激活的`push`与远端部署，最后评审OpsService。Remote Agent新能力在整个Program中继续冻结。
...
Artifact F0 cutover只给D0a增加一个pre-effect gate，不改变上述14-key schema、既有成功/错误bytes或其他D0a分支：exact五token由resident supervisor在任何`run_up`、Starting lifecycle generation/record/socket或legacy/lower effect前取得既有lifecycle `owner.lock`，并在同一gate内strict检查external top final、staging与next evidence。strict-valid PXMJ2 final，或可唯一归因于external operation的strict-valid A staging/canonical next，必须先验证owner/codec到足以可信读取其中PXDQ immutable config commitment：若该commitment与本次current config不等，优先固定`PXLC-LIFECYCLE-CONFIGURATION`/`managed-local lifecycle configuration authority changed`、exit 2；匹配时才固定返回原14-key顺序、`ok = false`、`profile = null`、`changed = false`、其他owner projection为null、`current_health_checked = false`、恰一项`PXLC-DEPLOY-REPLACE-REQUIRED`/`existing deployment state requires explicit replacement`、exit 2。两者都不得进入lower或创建lifecycle record。invalid、partial、mixed、cross-operation、identity不明，或无法可信读取commitment的top evidence固定为既有`PXLC-DEPLOY-LIFECYCLE`/`compiled local deployment did not reach the running owner generation`、exit 1；不得从raw offset猜config。若本invocation零effect已证明则`changed = false`，否则只能为null。检查通过只在final/staging/next都strict证明absent时成立；resident持续exclusive持有同一lifecycle gate穿过composition直到joined shutdown，禁止check-release TOCTOU或由D0a创建top scaffolding。

两份hardcoded D0a cutover expected JSON fixture固定为`artifact_f0_d0a_cutover_replace_required_v1.json`与`artifact_f0_d0a_cutover_lifecycle_v1.json`；下列各自是一行canonical compact JSON后恰一个LF，前者覆盖valid final/staging/next，后者覆盖pre-effect invalid evidence，effect无法证明的lifecycle negative只把同一schema中的`changed`改为null：

```json
{"schema_version":1,"command":"deploy","mode":"local","ok":false,"profile":null,"changed":false,"generation":null,"deployment_revision":null,"controller_snapshot_sequence":null,"runtime_apply_request_digest":null,"runtime_terminal_receipt_digest":null,"terminal_outcome":null,"current_health_checked":false,"diagnostics":[{"code":"PXLC-DEPLOY-REPLACE-REQUIRED","message":"existing deployment state requires explicit replacement"}]}
```

```json
{"schema_version":1,"command":"deploy","mode":"local","ok":false,"profile":null,"changed":false,"generation":null,"deployment_revision":null,"controller_snapshot_sequence":null,"runtime_apply_request_digest":null,"runtime_terminal_receipt_digest":null,"terminal_outcome":null,"current_health_checked":false,"diagnostics":[{"code":"PXLC-DEPLOY-LIFECYCLE","message":"compiled local deployment did not reach the running owner generation"}]}
```

`artifact_f0_d0a_cutover_configuration_v1.json`覆盖strict-valid final/staging/next中可信PXDQ commitment与current config不等，仍保持原14-key顺序且精确为下列单行加一个LF；config mismatch与codec corruption并存但commitment已不可信时必须回到上一份lifecycle fixture，不能凭raw bytes让configuration压过owner validation：

```json
{"schema_version":1,"command":"deploy","mode":"local","ok":false,"profile":null,"changed":false,"generation":null,"deployment_revision":null,"controller_snapshot_sequence":null,"runtime_apply_request_digest":null,"runtime_terminal_receipt_digest":null,"terminal_outcome":null,"current_health_checked":false,"diagnostics":[{"code":"PXLC-LIFECYCLE-CONFIGURATION","message":"managed-local lifecycle configuration authority changed"}]}
```

### Artifact F0 — A1 build/inspect/materialize 与 D0b fresh deploy

Artifact F0 首批且仅有以下六条 intended-public exact grammar；命令、选项与参数顺序逐 token 固定。六条surface及其owner APIs已在r606同一exact ref登记并完成joint candidate验证，但该ref尚未merge/release；前四条仍不取得独立A1 admission或发布状态，其有效验证只来自与后两条真实Controller/Runtime/TUI consumer共同通过的同一r606 gate：
...
`artifact build --output <ABS>` 的 `<ABS>` 是 non-root、lexical-canonical absolute artifact directory；整条既有父链必须无 symlink并由当前 uid/gid安全控制，immediate parent必须already-exist、strict且由no-follow descriptor pin住；mkdir前必须重新验证parent fd/identity并证明output leaf与derived staging basename都absent，build不得recursive mkdir或直接创建final leaf。whole-directory sibling staging的exact name固定为`.paraegox-artifact-build-v1-<64lowerhex>.initializing`（105 bytes），其中digest精确为`SHA-256("paraegox.artifact.build-output-path.sha256.v1" || u64_be(canonical_output_abs_utf8_len) || canonical_output_abs_utf8_bytes)`；digest必须是精确64字符lower-case hex，整个derived staging basename必须恰105 bytes且不得等于output leaf；native `NAME_MAX`/`PATH_MAX`与checked length在mkdir前失败时固定PATH/false。首次build只在该pinned parent fd上`mkdirat`这个fixed staging为0700/no-overwrite，fsync parent并只从same parent exact reopen empty staging；随后在staging内以`O_EXCL|O_NOFOLLOW|O_CLOEXEC`直接写`manifest.pxam`（精确206 bytes）与逐byte等于accepted source的`payload.bin`（1..64 bytes），两者均mode0600、regular、single-link且不得设置executable bit。staging本身是唯一temp，内部禁止`.next`、random temp、lock或extra entry；两file分别write-all/fsync/close/exact reopen，并验证staging为0700 real directory、两个file均0600 regular/single-link/exact bytes与exact two-entry set；再fsync、close staging并只从同一pinned parent exact reopen staging与两file。rename前必须再次strict重验parent fd/identity、config/output path、output仍absent、staging仍为同一identity与上述exact two-entry bytes，只可在same parent fd上`RENAME_NOREPLACE` staging→final，fsync并close/reopen parent，再只从该reopened parent exact reopen final与两file并逐byte复核，之后才返回`changed = true`。existing strict exact final且staging absent的replay必须在本调用依序fsync两file、final directory与immediate parent，并从同一reopened parent exact reopen final/pair全链后才成功返回false；这只收口可能停在rename后parent-fsync前的同一canonical publication，不改bytes/inode。任一步failure或durability ambiguity固定UNCERTAIN/null；conflicting final固定PATH/false。任何invocation开始前已存在的fixed staging，或final+staging，无论empty/partial/full/same bytes，都只做no-follow existence识别并分类`PXLC-ARTIFACT-UNCERTAIN`/`changed = false`，不读取authority、恢复、cleanup或overwrite；其metadata/path本身invalid则固定PATH/false。本调用创建staging后任一file/staging/parent failure、rename old/new或durability不明固定UNCERTAIN/null；parent/path/identity在effect后异常同样为UNCERTAIN/null；crash至多留下一个fixed quarantined sibling，不暴露partial final，也不生成unbounded temp。build不读写state root、ArtifactStore、Controller或Runtime。`artifact inspect` 只接受显式manifest/payload路径；无论成功或失败都不得创建、打开build staging、写入、rename、删除、chmod/chown任何对象，也不创建domain state。
...
- local layout 只有在本 invocation 的完整 config、输入 pair、A0、compatibility 与所有 path preflight 已通过后，才可创建或验证 `<state_root>`；它必须是当前 uid/gid 拥有、mode 0700、全链 no-symlink 的 real directory。若leaf尚不存在，只允许其immediate parent已经strict存在并由no-follow descriptor pin住；不得recursive mkdir。materializer在同一parent fd上以no-overwrite `mkdirat`创建mode0700 leaf，fsync parent，再只从该parent exact reopen leaf并复核dev/ino/uid/gid/mode/no-symlink与empty entry set，随后重解析config path并证明仍指向同一identity；只有这条durability链完成后才把leaf pin为下文`state_root`并创建store staging。mkdir/parent-fsync/reopen的old/new仍不明但attribution可信时固定`PXLC-ARTIFACT-UNCERTAIN`/`changed = null`且绝不draw instance或发布authority；parent/leaf identity、ownership或path失真在effect前按PATH/false，effect后按OWNER/null。crash recovery只在identity draw前可对exact empty leaf补齐parent fsync/reopen，或在leaf确证absent时重新mkdir；它不赋予store identity。既有strict-safe state root只验证、不chmod或改写。durably-created empty leaf只是non-authoritative scaffolding，不生成ArtifactStore identity、counter、operation或Receipt，也不单独令owner `changed = true`；其后确定零owner mutation的failure仍可false，但scaffolding publication/durability不明必须null。materialize 的最长 derived suffix 固定为 178 bytes；Linux `PATH_MAX = 4096` 下 `<state_root>` 的 UTF-8 byte length 必须 `<= 3917`，否则在任何 filesystem effect 前返回既有 `PXLC-STATE-ROOT-TOO-LONG`。query 无论成功、NotFound或失败都不得创建 state root、final/staging store root、lock、snapshot next、object directory或pair temp；state root absent或strict empty时只返回NotFound/false。
- Accepted 初始化不创建 empty snapshot。final absent时，strict-valid empty staging、只含exact zero-length 0600 `artifact.lock`的lock-only staging，以及只含该lock与strict empty 0700 `objects/`的lock+objects staging，都是non-authoritative publication prefix，不携带store/request identity。materializer全程使用preflight已pin并复核dev/ino/uid/gid/mode的同一`state_root` fd，所有mkdir/open/rename/fsync/reopen都descriptor-relative：mkdir staging→在staging内`O_EXCL` create并fsync lock、取得exclusive→锁内重验final absent及staging identity→fsync并exact reopen staging parent/lock→mkdir并fsync empty objects→再次fsync并从pinned state root exact reopen staging parent，再只从该reopened parent重开并复核lock+objects identity与exact entry set。还必须fsync/reopen同一pinned state-root parent，证明staging entry durable；只有该lock+objects全链成立后才执行一次nonzero store-instance draw并create任何snapshot byte。首个 authoritative `artifact.snapshot` 必须已经包含首个 exact PXAQ/PXAA、该random nonzero `artifact_store_instance`、nonzero immutable config commitment、`snapshot_sequence = 1`、operation high-water/count均为1、object high-water/count均为0。snapshot以no-overwrite create/write/fsync/close/exact reopen后再次fsync/reopen staging并复核lock+objects+snapshot；rename前复核pinned state-root fd identity且config path仍解析为同一state root，只用同一fd上的`RENAME_NOREPLACE`发布整个staging，fsync/reopen同一state-root fd，再只从该fd exact reopen final root，并从reopened final root逐项重开复核lock、snapshot与objects。这一次 directory publication 才是首个 ArtifactStore authoritative/operation mutation；任何state-root path replacement、final-child swap或descriptor drift都固定OWNER/null，不能声称publication durable。
- final absent时，query看见strict-valid empty staging固定线性化在lock创建前，返回NotFound/`changed = false`且零创建；看见strict-valid lock-only或lock+objects staging只对既有lock一次try shared，`WouldBlock`固定`PXLC-ARTIFACT-UNCERTAIN`/false，取得后重验final、snapshot absent与相应fixed prefix entry set仍成立才返回NotFound/false。live materializer只有在exclusive lock内且empty objects durable后才可创建任一snapshot byte，因此query会先被lock阻断；crash遗留的short/partial/noncanonical snapshot、snapshot present但lock/empty objects缺失、invalid prefix metadata或extra entry固定`OWNER`。identity、layout、snapshot checksum/bytes、empty objects与exact PXAQ全部可证明的完整staging，只可由same exact materialize request在staging exclusive lock下完成publication；query遇到这种可归因于所查operation的same-request staging时返回下文`UNCERTAIN`且不publish。different/cross-request、unknown authority bytes、final与staging同时存在全部`OWNER`，不得猜store instance、operation、counter或归属。final root已经存在时empty、lock-only、lock+objects、snapshot缺失/损坏或不可strict reopen一律`OWNER`，绝不是virgin store，不能重新初始化或补empty snapshot。
- stable final root 的 entry set 精确为 `artifact.lock`、`artifact.snapshot`、`objects`；transaction期间只可额外出现 fixed `.artifact.snapshot.next`。final/staging owner root、`objects/`与每个object directory都必须是当前uid/gid拥有、mode0700、no-symlink的real directory；lock、snapshot、next、manifest、payload与pair temp都必须是当前uid/gid拥有、mode0600、regular、single-link文件，每次访问以no-follow descriptor复核device/inode/owner/mode/nlink/length。`objects/` 只允许 snapshot 索引的最多64个canonical object directory，外加至多一个unindexed canonical child；该child只能是两个互斥形态之一：(a) snapshot最后operation已有exact PXMU、尚无PXAW且其exact PXAQ/PXAK没有matching PXAV的materializing crash child，其名称必须由该PXAQ/PXMU唯一归因，entry set只能是`.manifest.pxam.next`、`.payload.bin.next`、`manifest.pxam`、`payload.bin`的任意fixed subset（包括empty），所有present entry仍须通过上述strict metadata/identity/length/bytes验证；或(b) snapshot最后operation已有exact PXMU、PXAV严格absent并进入object-publication-uncertain PXAW-U时，与其唯一归因的incomplete/quarantine child及fixed pair temp；该U可以尚无PXAX，也可以已有matching PXAX-U，后者仍保留directory/bytes/block。形态(a)不是extra、不是quarantine，固定bit0为0且`quarantine_bytes = 0`；形态(b)固定bit0为1。若operation的PXAQ/PXAK已经命中任一indexed PXAV，则任何额外child/temp/remnant都非法，并且该operation只能按strict snapshot history终结M或E；F0不允许任何PXAV-present U。两个形态不能并存；unknown/cross-operation child、第二个unindexed child或fixed subset外entry全部精确分类`OWNER`。enumeration唯一语义是验证上述allowlist并拒绝extra/unknown entry，绝不从目录反推 object table、operation、counter或recovery结论。`artifact.snapshot` 是唯一 owner record、store identity、config commitment与两个 high-water 的事实来源；不存在独立 operation、Receipt、journal或counter file。PXAQ/PXAA/PXMU/PXAV/PXAW/PXAX只是snapshot内的canonical records。
- 最后一条operation已有exact PXMU、尚无PXAW且其PXAQ/PXAK唯一命中一个indexed PXAV时，有两种合法matching-PXAV crash-prefix：该PXAV是本operation刚由object successor发布但尚未有PXAW的M prefix（1104-byte object-terminal snapshot），或该PXAV已由更早terminal operation引用、当前operation尚未签PXAW-E的E prefix。两者都只允许该PXAV索引的exact stable `manifest.pxam`/`payload.bin` pair；全链必须strict验证，且不得出现为当前operation新建的child、pair temp、remnant或indexed directory中的extra entry。任何额外child/temp/remnant都精确分类`OWNER`，不得借matching PXAV吸收或清理；query不推断最终M/E，只返回下文固定materializing envelope，只有mutating recovery按锁存的recovery-start事实终结M或E。
...
`materializing_digest = SHA-256("paraegox.artifact.materializing.sha256.v1" || frame[0..208])`。同一 admission 最多发布一份 exact PXMU。restart 必须先按下文规则收口 snapshot-next，再以durable PXMU锁存recovery-start facts并恢复同一operation，不得先改写为U或生成新id/sequence：只从canonical PXAQ/PXAA中的exact PXAK与owner-derived fixed object location执行一次bounded no-follow reopen，不扫描目录、不接受path fallback、不重放unknown temp write、不覆盖/删除任何既有对象。recovery-start snapshot已含matching exact PXAV且canonical pair/identity/high-water/durability全链可证明时，必须再按snapshot table history唯一分类：若该PXAV未被任何更早terminal PXAW引用，并且依照“最多一个unreferenced PXAV且只属于最后incomplete operation”的invariant唯一归属于本operation，则这是本operation已发布PXAV、尚未PXAW的M prefix，必须终结PXAW-M/PXAX-M；若该PXAV已被更早terminal operation引用，则是E prefix，终结PXAW-E/PXAX-E。snapshot尚无PXAV但canonical full pair可经exact reopen、file refsync与object-directory fsync重新证明时，必须在一个successor内按原object high-water规则加入exact PXAV并终结M。三种情况不得互换M/E，也不得根据目录或不存在的creator field猜测；ambiguous/multiple/history drift固定OWNER。只有exact operation attribution以及root/parent/child/file owner identity全部可证明、但partial pair的effect/durability outcome仍不确定时，才可在完成下文remnant durability/reopen全链后以保留全部verified facts的PXAW-U、随后matching PXAX-U终结原operation并归因quarantine；owner identity或operation attribution本身无法证明时精确分类`OWNER`，绝不签发PXAW-U/PXAX-U。recovery不能把PXAA/PXMU、文件存在或清理成功单独解释成completed object。
...
`operation_terminal_digest = SHA-256("paraegox.artifact.materialization-terminal.sha256.v1" || frame[0..272])`。`M|E` 要求 PXMU 与可 exact reopen 的 PXAV digest 都 nonzero。`M`精确表示本operation正常/恢复时新发布PXAV，或recovery-start已见到一份尚未被任何更早terminal PXAW引用、且由snapshot invariant唯一归属于最后incomplete本operation的unreferenced PXAV；`E`精确表示matching PXAV已被更早terminal operation引用。PXAV不携带creator operation id，分类只用strict snapshot history/reference invariant，不能仅以“recovery-start时PXAV存在”判E、按目录猜测或事后交换。`F` 的PXAV digest必须全零，并且owner必须证明没有任何pair temp/final、incomplete directory或quarantine/remnant存在；它可以保留已durable PXMU，但此时还必须证明PXMU后发生了零pair effect。`U`固定要求PXMU digest nonzero、PXAV digest全零、bit0=1，并存在至少一个可由本operation exact归因且通过下文durability/reopen全链的unindexed child/remnant；owner/identity/attribution任一无法证明都必须分类`OWNER`且不得生成PXAW-U/PXAX-U。F0没有PXAV-present U或bit0=0 nonblocked U：matching PXAV strict history必归M/E，PXAV successor publication ambiguity归outer `PXLC-ARTIFACT-UNCERTAIN`/null，identity/durability失真归OWNER；PXAV absent完整pair可refsync/reopen则加入PXAV后M，零pair effect/remnant则F。`F|U`都不删除admission/progress；最后U的matching PXAX-U不清flag/bytes/block。
...
`state_flags`只允许bit0 `OBJECT_PUBLICATION_BLOCKED`，其他bit必须为0。bit0为0严格要求`quarantine_bytes = 0`且不允许U；bit0为1允许`quarantine_bytes = 0..=540`，但snapshot最后一条operation必须有exact PXAQ/PXAA/PXMU、PXAV严格absent，且是可由canonical unindexed child逐项归因的object-publication-uncertain `PXAW-U`；filesystem中还必须实际存在至少一个由该U canonical归因的entry（empty child directory或zero-byte temp允许bytes为0），即使已有exact `PXAX-U`也不得清flag。U对应PXOP只能是PXAX absent的flags3或PXAX present的flags7；flags2/6只允许PXMU absent的F，flags3/7在bit0=0时也只可表示PXMU present的F，不能表示nonblocked U。nonzero `quarantine_bytes`精确等于该最后U所归因的unindexed regular temp/final manifest与payload byte length之和，每个byte恰计一次。任一PXAV present都要求bit0=0/quarantine=0，indexed stable pair只由`sum(indexed_object(...))`计费且其directory不得有extra temp/remnant，否则OWNER；capacity text中的indexed+extra row只测试isolated component calculator，绝不表示canonical state。directory、zero-length lock、snapshot与filesystem block allocation都不计。bit0为1同时是严格的operation-admission gate：owner在读取并验证snapshot后、写入任何新PXAQ/PXAA或推进operation count/high-water前，必须拒绝每个new/different operation，即使其PXAK精确命中已有indexed PXAV、原本可形成E；唯一允许的materialize路径是同一最后quarantine-U operation + exact same PXAQ：`PXAX-U` absent时只可补齐matching `PXAX-U`，present时只可做pure terminal replay；read-only query仍允许，且这些路径都不得清除bit或归因bytes。该new/different-operation admission reject固定为`PXLC-ARTIFACT-CAPACITY`、exit 2、`ok = false`、`changed = false`，保留已strict parse的input operation id，`state`/O/MR均为null；same operation id + different request仍按更早的conflict规则拒绝。bit0为0但bytes nonzero或U present、bit0为1但无实际attributed entry或无上述最后U、任一PXAV present、归因到非最后operation、漏计/重复计、`quarantine_bytes > 540`，以及unknown/无法归因/identity不明的temp或partial都拒绝整份snapshot并令owner失败关闭，不能靠设置flag吸收。negative matrix必须分别从1216/1456-byte U/block fixture删除PXMU、把flags3/7改为2/6并重算entry/body/frame length、`accounted_rest_bytes`与checksum，同时保留bit0及其canonical attributed entry；两份均须在零mutation下精确映射`PXLC-ARTIFACT-OWNER`。checksum精确为`SHA256("paraegox.artifact.store-snapshot.sha256.v1" || u64_be(160) || header[0..160] || u64_be(body_len) || body)`；decoder先做bounded length/read与exact EOF，再按offset、reserved、flag、count、body和cross-frame规则strict decode、重算checksum，并要求canonical re-encode逐byte等于输入，不能接受short/trailing、unknown version/algorithm或checksum后字段。
...
`artifact.lock`固定为owner/staging root中的zero-length、mode0600、当前uid/gid、regular、single-link、no-follow `O_CLOEXEC`文件。除上文strict empty-staging query线性化外，materialize/query都必须先canonical strict-open并验证既有lock metadata/identity/layout，再在本invocation任何authoritative effect前只尝试一次锁：mutation取exclusive，query取shared，不等待、重试或另建lock；只有materializer可在strict empty staging中以`O_EXCL`创建该lock，query绝不创建。strict-valid owner、lock-only staging或lock+empty-objects staging上的`WouldBlock`/`EWOULDBLOCK`统一映射`PXLC-ARTIFACT-UNCERTAIN`；其他lock syscall failure映射`PXLC-ARTIFACT-IO`；lock metadata/identity/layout不合法映射`PXLC-ARTIFACT-OWNER`，三者不得互换。lock abstraction不暴露raw fd，所有handle/clone的`Drop`必须显式unlock而不是只依赖close。任何fork/spawn/exec、supervisor或workload创建前，parent必须显式unlock并drop全部clone；child即使意外继承，也必须在进入任何业务路径前显式unlock并close，不能把`CLOEXEC`当作fork safety证明。

restart mutation在exclusive lock内必须先收口canonical snapshot-next，再从settled snapshot的PXMU锁存recovery-start state。snapshot已有matching exact PXAV且pair全链可证明时，unreferenced且唯一归属于最后incomplete本operation的1104-byte object-terminal prefix终结M，已被更早terminal operation引用的prefix才终结E；snapshot尚无PXAV而recovery遇到已有canonical child或任一final pair entry时，必须先fsync当前pinned `objects/` parent，close并exact reopen该parent，再只从该reopened parent exact reopen child并复核parent/child device/inode/owner/mode/nlink与entry set；只有此前置证明成立，才可对exact complete final pair逐文件refsync、fsync child、经同一个reopened parent再次exact reopen child/pair并复验全部bytes，随后在同一object successor追加PXAV/high-water再终结M。partial pair或effect不确定只能在identity与attribution均可证明、PXAV absent时进入bit0-U候选；任何bit0-U successor在durable引用attributed remnant前，还必须依序fsync每个attributed regular temp/final file、child与pinned `objects/` parent，close该parent并从仍pinned final root exact reopen `objects/` parent，再只从该reopened parent exact reopen child及全部attributed entry，复核root/parent/child/entry identity、entry set、bytes与quarantine accounting。上述任一fsync/reopen、identity、attribution或durability证明失败都精确分类`OWNER`，不得发布U或PXAV；只有完整证明后的unindexed remnant才能按上述规则由最后operation记录quarantine/block。virgin（final/staging都absent）以及上文strict empty、lock-only、lock+objects staging的稳定pre-authority读都固定NotFound/`changed = false`，valid store中operation absent同样NotFound。valid、attributable same-request complete staging，或canonical same-operation `N + 1` next且active尚无terminal，精确映射`PXLC-ARTIFACT-UNCERTAIN`；partial/noncanonical authority bytes、unknown/cross-request staging或next、final+staging、任何extra entry以及root/lock/snapshot的owner/mode/link/identity/layout/codec/checksum strict failure，精确映射`PXLC-ARTIFACT-OWNER`。若所查operation已有exact PXMU、尚无PXAW/matching PXAV且其materializing child absent或精确符合上述唯一归因/fixed-subset形态，query只做strict read/reopen验证后固定返回`materializing`、O、MR null、`[]`、exit 0、`changed = false`；它不得fsync、恢复、发布PXAV/PXAW/PXAX或清理任何entry。该形态中的unknown/cross-operation/second child、subset外entry，或PXAQ/PXAK已命中indexed PXAV却仍有额外child，都精确分类`OWNER`。所有query路径的`changed = false`；materialize的`OWNER.changed`只允许三种互斥结果：本invocation零mutation已证明时为false；已把initial canonical whole-directory `RENAME_NOREPLACE` + state-root fsync + final root/lock/snapshot/objects exact reopen，或一份canonical successor rename + owner-root fsync + final-snapshot exact reopen，全部证明为本invocation事实，且随后OWNER检查strict read-only并证明零additional effect时才为true；只要随后child/temp/write/fsync/remnant的effect set或durability有任何不明，即使较早publication已证明，也必须为null。initial或successor old/new publication ambiguous固定`PXLC-ARTIFACT-UNCERTAIN`/null，不是OWNER。`OWNER`除已strict parse并回显的input operation id外，`state`、object ref与Receipt ref始终null。query不得补PXAX、fsync、cleanup、recovery或任何successor；PXAW已存在而PXAX缺失时按下文四行返回相应terminal且Receipt为null，只有materialize可推进PXAX。v1不做online migration、version fallback或content sniffing；未来格式只能由offline successor root迁移。

matching-PXAV crash-prefix query同样只读：最后operation已有exact PXMU、无PXAW且其PXAQ/PXAK唯一命中indexed PXAV时，无论该PXAV是本operation刚发布的M prefix还是更早operation已terminal引用的E prefix，都必须strict验证`PXMU → PXAV → PXAA → PXAQ → PXAM/payload`及该indexed directory只有exact stable pair，并固定返回`materializing`、O、MR null、`[]`、exit 0、`changed = false`。query不得推断M/E、签发PXAW/PXAX、fsync或清理；任何额外child/temp/remnant、多个matching PXAV或stable pair链漂移都精确分类`OWNER`。

capacity accounting只接受以下checked regular-file公式。PXAY body最大值是`64 + 64 * 192 + 1024 * 1200 = 1241152` bytes，PXAZ snapshot最大值是`192 + 1241152 = 1241344` bytes。canonical snapshot frame-length vectors依次为：initial admitted `672`、materializing `912`、加入PXAV后的object-terminal `1104`、加入PXAW但尚无Receipt的materialized-terminal `1408`、加入PXAX后的materialized-receipt `1648`、PXMU absent的failed-terminal/failed-receipt `976/1216`、PXMU present但零pair effect/remnant的failed-after-materializing terminal/receipt `1216/1456`，以及复用同一object的第二个E operation materializing/terminal/receipt `2304/2608/2848` bytes；两份U/block frame仍分别是`1216`与`1456`。这些只是frame bytes，不能冒充header中的logical accounting。每次decode与successor都必须重算并要求header的`accounted_rest_bytes = current_snapshot_frame_bytes + sum(indexed_object(manifest_len 206 + payload_len 1..64)) + attributed_quarantine_regular_file_bytes`；本golden的payload精确20 bytes，因此object-terminal/materialized-terminal/materialized-receipt/second-E-materializing/second-E-terminal/second-E-receipt的`accounted_rest_bytes`分别是`1330`、`1634`、`1874`、`2530`、`2834`、`3074`，PXMU absent的failed-terminal/failed-receipt为`976/1216`，PXMU present的failed-after-materializing terminal/receipt为`1216/1456`，而admitted/materializing与zero-quarantine且尚无PXAV的两份U/block仍分别等于其`672`、`912`、`1216`、`1456` frame length。`1259164 = 1241344 + 64 * (206 + 64) + 540`只是这些逻辑regular-file components逐项取cap所得的componentwise conservative accounting upper bound，不声明该组合可由一份canonical snapshot或合法preflight history同时达到；它不包含directory、zero-length `artifact.lock`、inode metadata、filesystem block/slack或`du`物理占用。transaction calculator按`accounted_before + candidate_snapshot + new_pair_bytes_not_already_counted + known_owner_temp_bytes_not_already_counted`逐项checked；fixed pair publication使后两项的conservative combined maximum为270，因此`2500778 = 1259164 + 1241344 + 270`只是componentwise conservative transaction upper bound，同样不包含directory、zero-length lock、inode metadata或filesystem block allocation，不能声称是physical peak或constructible state。计算结果`<= 8388608`接受，`> 8388608`或任一overflow拒绝；64/1024 count caps严格支配8-MiB defense ceiling，所以public v1不能通过合法history自然耗尽它。8-MiB边界只用pure/injected checked calculator测试，禁止用padding、超额quarantine、伪造history或放宽count caps制造“合法耗尽”。

materialization hardcoded shared goldens 全部位于 `tests/fixtures/wire/`，文件名与内容边界冻结为：`artifact_f0_pxam_v1.hex`（206 bytes）、`artifact_f0_pxak_v1.hex`（72 bytes）、`artifact_f0_pxaq_v1.hex`（176 bytes）、`artifact_f0_pxaa_v1.hex`（208 bytes）、`artifact_f0_pxmu_v1.hex`（240 bytes）、`artifact_f0_pxav_v1.hex`（192 bytes）、`artifact_f0_pxaw_materialized_v1.hex`（304 bytes）、`artifact_f0_pxaw_already_materialized_v1.hex`（304 bytes）、`artifact_f0_pxaw_failed_v1.hex`（304 bytes）、`artifact_f0_pxaw_uncertain_v1.hex`（304 bytes）与对应四份 `artifact_f0_pxax_*_v1.hex`（各240 bytes）。snapshot fixtures固定为`artifact_f0_pxaz_admitted_v1.hex`（672 bytes）、`artifact_f0_pxaz_materializing_v1.hex`（912 bytes）、`artifact_f0_pxaz_object_terminal_v1.hex`（1104 bytes）、`artifact_f0_pxaz_materialized_terminal_v1.hex`（1408 bytes）、`artifact_f0_pxaz_materialized_receipt_v1.hex`（1648 bytes）、`artifact_f0_pxaz_failed_terminal_v1.hex`（976 bytes、flags2、snapshot sequence2）、`artifact_f0_pxaz_failed_receipt_v1.hex`（1216 bytes、flags6、snapshot sequence3）、`artifact_f0_pxaz_failed_after_materializing_terminal_v1.hex`（1216 bytes、flags3、bit0=0、quarantine=0、snapshot sequence3）、`artifact_f0_pxaz_failed_after_materializing_receipt_v1.hex`（1456 bytes、flags7、bit0=0、quarantine=0、snapshot sequence4）、`artifact_f0_pxaz_already_materialized_materializing_v1.hex`（2304 bytes、second-operation flags1、snapshot sequence7、logical accounting2530）、`artifact_f0_pxaz_already_materialized_terminal_v1.hex`（2608 bytes、second-operation flags3、snapshot sequence8）、`artifact_f0_pxaz_already_materialized_receipt_v1.hex`（2848 bytes、snapshot sequence9）、`artifact_f0_pxaz_uncertain_blocked_v1.hex`（1216 bytes：object_count0、PXMU present、PXAW-U、PXAX absent、flags3、bit0=1、quarantine=0）与`artifact_f0_pxaz_uncertain_receipt_blocked_v1.hex`（1456 bytes：同一U加入matching PXAX-U、flags7，bit0/quarantine归属不变）。两份failed-after-materializing fixture必须保留exact PXMU/PXAW-F materializing-digest关联且证明零child、pair、temp与remnant；两份U/block fixture的filesystem harness都必须实际建立canonical attributed empty child或zero-byte temp，snapshot bytes本身不能伪造entry存在。

五份PXAW-only query expected JSON固定为`artifact_f0_query_pxaw_only_materialized_v1.json`、`artifact_f0_query_pxaw_only_already_materialized_v1.json`、`artifact_f0_query_pxaw_only_failed_v1.json`、`artifact_f0_query_pxaw_only_failed_after_materializing_v1.json`与`artifact_f0_query_pxaw_only_uncertain_v1.json`；每份都是按上述top-level key order的一个compact object加单一LF，复用对应hardcoded PXAW/PXAQ的exact operation id与O，`changed=false`、`materialization_receipt_ref=null`，并逐项冻结五行各自ok/state/diagnostic/exit语义。输入分别是1408-byte M、2608-byte E、976-byte PXMU-absent F、1216-byte PXMU-present F与1216-byte U hardcoded snapshot；两份F readback必须给出逐byte相同的public failed envelope，但各自strict证明PXMU absence/presence与PXAW materializing digest的对应关系，不允许query mapper直接拼nested frame或调用production encoder自证outer codec。materializing crash query另固定四份expected JSON：`artifact_f0_query_materializing_empty_child_v1.json`、`artifact_f0_query_materializing_temp_v1.json`、`artifact_f0_query_materializing_single_final_v1.json`与`artifact_f0_query_materializing_full_pair_v1.json`；harness分别建立唯一归因的empty child、fixed pair-temp subset、single final与complete final pair，四份都逐byte冻结同一个`ok=true`、`changed=false`、exact operation id、`state="materializing"`、O、MR null、`diagnostics=[]` envelope与exit 0，并证明query前后snapshot、directories与全部entry逐byte/identity不变且没有fsync/cleanup/recovery。

matching-PXAV query expected JSON另固定两份：`artifact_f0_query_materializing_object_terminal_v1.json`直接消费本operation刚发布PXAV、尚无PXAW的1104-byte M-prefix snapshot；`artifact_f0_query_materializing_existing_object_v1.json`直接消费2304-byte `artifact_f0_pxaz_already_materialized_materializing_v1.hex`，其中second operation的exact PXMU命中由更早terminal M operation索引的唯一PXAV/stable pair，尚无当前PXAW/PXAX。两份expected envelope都精确为`materializing`/`changed=false`/O/MR null/`[]`/exit 0，query不能据此选择M/E；1104 M prefix与2304 E prefix的history/operation-sequence互换必须strict reject，分别在indexed directory或另一个child注入extra temp/remnant的negative也必须精确OWNER，query前后全部owner/filesystem bytes与identity不变。

`artifact_f0_pxop_presence_v1.txt`逐行冻结上述六个flags/length组合；`artifact_f0_store_capacity_v1.txt`只hardcode pure/injected checked calculator的component input/result、cap与overflow，不伪造canonical snapshot或history：包括全部公式、snapshot-frame `1241344` accept/`1241345` reject、`objects-64`（64 accept/65 reject）、`operations-1024`（1024 accept/1025 reject）、`quarantine-540`（540 accept/541 reject）、indexed object与额外temp各计一次的isolated component row，以及`8388608` accept/`8388609`与arithmetic overflow reject；这些isolated rows不得被组合后宣称为constructible maximum。`blocked-zero-byte`与`last-u-attribution`语义由上述两份PXAV-absent U/block snapshot及filesystem harness冻结：bit0=1、quarantine=0时必须存在canonical attributed empty child或zero-byte temp，无entry拒绝；PXAW-U前与matching PXAX-U后保留相同flag/bytes/block，non-last/mismatch/PXAV-present均拒绝。另有negative证明任一PXAV state固定bit0=0/quarantine=0、indexed directory只有stable pair，extra temp/remnant必须OWNER；calculator的indexed+extra row不构成canonical state。`artifact_f0_text_refs_v1.txt`冻结exact object/ref行，所有text fixture只有单一末尾LF。PXAA/PXMU/PXAW/PXAX happy vector的ArtifactStore-global operation sequence固定为1，PXAV object sequence固定为1；bounded-recovery golden必须分别hardcode“recovery-start已有unreferenced且唯一归属本operation的1104-byte PXAV→M”、“recovery-start matching PXAV已被更早terminal引用→E”与“recovery-start无PXAV、由本次恢复新发布→M”，并拒绝交换后的terminal bytes；独立successor vector必须hardcode sequence 2、same-operation/same-object replay仍为1以及`u64::MAX` preflight reject。所有frame/snapshot/JSON golden必须由独立Rust与Python decoder消费且expected bytes hardcoded，不能在测试中调用production encoder生成期望值；negative matrix逐项覆盖flags、table/sequence ordering、config、checksum/domain、short/trailing、max+1、cross-frame digest/store/id/ref、temp归因、restart M/E、fork/lock继承与cross-request next。
...
432..464  reserved_superseding_desired_head_digest[32] = all-zero in F0
464..496  operation_record_digest[32]
```

`operation_record_digest = SHA-256("paraegox.deployment.external-operation-record.sha256.v1" || frame[0..464])`。`record_sequence` 是该 operation 内从 1 开始、每个 successor 精确加一的局部序列，不是 Controller 全局 journal offset。首条 `C` 使用 all-zero previous record，要求 revision、snapshot、desired head nonzero，Runtime/generation字段全零，outcome `N`；`P` 要求前述 committed 字段与 runtime apply digest、generation nonzero，outcome `N`；`R` 要求所有 committed/applying 字段与 Runtime terminal digest nonzero，outcome `R`。reserved superseding digest在每一种F0 state都必须全零，decoder拒绝任一nonzero byte，不能留作future enabler。PXDQ/PXDK以及全部PXDM/PXDO的16-byte operation字段始终是同一个deployment operation id；这些frame不复制或冒充PXAR内的Runtime ApplyOperationId。PXDM/PXDO只通过`runtime_apply_request_digest`关联Runtime owner：该字段一旦nonzero就必须逐byte等于durable exact PXAR12的Envelope request digest以及关联PXMT的request digest；`runtime_terminal_receipt_digest`一旦nonzero就必须等于该exact PXMT terminal Receipt digest。`F|U` 必须逐项保留前一 verified record 的所有 nonzero 字段，只允许把本次新获得且验证相关的后续字段从 zero推进到 nonzero，不得清空或改写；outcome分别为`F|U`。若 failure/uncertainty 发生在 PXDK 后、首条 `C` 前，第一份 PXDM 可以是 `F|U`，previous record与所有尚未产生的 progress 字段全零；这不是“未 admission”。首份 PXDM 的 previous record全零，之后必须精确引用同 operation 的前一 canonical PXDM；任何 operation 都从自己的 sequence 1开始且不能串接另一 operation 的 digest。`R|F|U`均terminal且不接受successor。replacement/supersede必然需要different operation与new desired，未被F0 fresh-only/ADR-0011授权；R0必须定义能同时保存old history与new current operation的显式wire/store successor，不能在PXDM/PXDO v1或PXMJ2内重新解释reserved bytes。

`PXDO` v1 是 fixed 432-byte terminal DeploymentController Receipt，只允许 `R|F|U`，并从 terminal PXDM逐字段复制 verified facts：
...
320..352  runtime_terminal_receipt_digest[32] or all-zero
352..368  lifecycle_generation[16] or all-zero
368..400  reserved_superseding_desired_head_digest[32] = all-zero in F0
400..432  deployment_receipt_digest[32]
```

`deployment_receipt_digest = SHA-256("paraegox.deployment.external-receipt.sha256.v1" || frame[0..400])`。`receipt_sequence` 是 Controller owner 在 terminal PXDO durable publication时分配的大于零、全 store 单调递增且不复用的 Receipt 序列；它不是 PXDM 的 operation-local record sequence。PXDO 的 optional-zero字段必须与 terminal PXDM逐 byte一致，reserved superseding digest对`R|F|U`一律全零；`R` 要求revision/snapshot/desired/runtime request/runtime terminal/generation全nonzero，`F|U`遵循PXDM verified-prefix preservation。canonical text `DeploymentReceiptRefV1` 固定为 `pxdor1:<64-lowerhex-controller-store-instance>:<canonical-decimal-receipt-sequence>:<32-lowerhex-operation-id>:<64-lowerhex-deployment-receipt-digest>`；它必须 canonical reparse/re-encode并经 Controller typed query取得 exact PXDQ/PXDK/PXDM/PXDO，不能从 JSON、desired 文件或 Runtime PXMT 自行合成。

D0b hardcoded shared goldens 同样位于 `tests/fixtures/wire/`，冻结为 `artifact_f0_pxdq_v1.hex`（288 bytes）、`artifact_f0_pxdk_v1.hex`（240 bytes）、`artifact_f0_pxdm_committed_v1.hex`、`artifact_f0_pxdm_applying_v1.hex`、`artifact_f0_pxdm_active_ready_v1.hex`、`artifact_f0_pxdm_failed_v1.hex`、`artifact_f0_pxdm_uncertain_v1.hex`（各496 bytes）、对应三个 terminal outcome 的 `artifact_f0_pxdo_{active_ready,failed,uncertain}_v1.hex`（各432 bytes）与加入 `artifact_f0_text_refs_v1.txt` 的 exact `pxdor1` 行。PXDK happy vector的Controller-global admission sequence固定为1；owner-private pure high-water successor vector（不构成F0第二个public admission）必须hardcode 2、same id replay仍为1与`u64::MAX` preflight reject。实现批还必须 hardcode `execution_profile_commitment` 与四个 D0b digest domain；golden、state transition与 frozen JSON expected value不能由同一 production encoder/mapper共同生成，且每份PXDM/PXDO golden都证明reserved superseding digest全零、single-bit nonzero strict reject。
...
168..170  PXDM count = u16_be(0..=3)
170..172  PXDO count = u16_be(0..=1)
...
componentwise decoder/allocation upper bound精确为 `192 + 288 + 240 + 2758 + 2506 + 6630 + 2048 + 3*496 + 1*432 + 32 = 16614` bytes；它不声称存在16614-byte canonical frame，cap内noncanonical nested PXAR12仍由strict decode拒绝。超出任一分段或该总allocation上限都在decode/admission时失败关闭。PXMJ2 header 的 `controller_snapshot_sequence` 从phase A的1开始，每个durable successor精确加一且不跳号；它是owner snapshot序列，不等于PXDM/PXDO内的committed snapshot pin。phase C header精确为2，PXDM-C的`controller_snapshot_sequence`也精确为2；从此以后每一份PXDM successor与由它签发的每一份PXDO都必须保留首次C值2，即使当前PXMJ2 header已推进到P=3、R=4或后续F/U sequence。pre-C F/U没有committed snapshot，其PXDM/PXDO该字段必须为0。`admission_high_water` 等于latest PXDK admission sequence，Artifact F0恒为1；`receipt_high_water` 为0或latest PXDO Receipt sequence，F0唯一terminal receipt为1。

`controller_store_instance` 的producer只属于top-level Controller owner：在全部A-phase preflight通过、owner lock已持有且尚无durable PXMJ2-A/PXDK时，构造A candidate前恰执行一次bounded 32-byte CSPRNG draw，并拒绝all-zero结果，不retry/reseed。它不得复制或派生ArtifactStore instance、Runtime store instance、deployment/materialization/Runtime operation id、temporal id或其他identity；不同typed namespace的raw bytes偶然相同不新增public reject，也不能被解释为alias。candidate只有随exact PXDK进入durable PXMJ2-A同一atomic publication后才取得Controller store identity：A从未durable时candidate没有owner意义，strict reopen证明A absent后下一次尝试可以重新draw；A publication old/new ambiguous时必须先strict reopen并完成分类，不能先draw，若恢复durable A就只复用其bytes，仍不能证明时失败关闭。A一旦durable，其PXMJ2 header与PXDK中的exact instance就是唯一事实源，所有PXMJ2 successor、PXDM、PXDO、query/replay及`pxdor1`都必须逐byte复用，绝不重新抽取、复制或派生。semantic ledger的`deployment.controller_store_instance_hex = d0…d0`只允许test injection生成hardcoded fixture；production不得读取ledger或把该常量当fallback。

mutable PXMJ2 frame checksum只证明该snapshot，绝不能用作cutover marker digest。immutable marker digest在phase A admission commit时精确派生为：
...
- `F|U`：phase必须等于最后一份PXDM state，并有恰一份同outcome PXDO。若从A直接在C前终结，则terminal header sequence为2，PlanContent/PXTE/PXAR可全部absent且revision、committed snapshot pin、desired/runtime digest保持零，PXDM只含`[F]`或`[U]`；但若Starting lifecycle record已durable，PXDM/PXDO必须保留该record的exact nonzero lifecycle generation，只有record尚未durable时该字段才为零。若在C/P后终结，header按当前durable prefix精确加一，必须保留全部已durable三段/header/PXDM prefix并只追加对应terminal，所有PXDM/PXDO的committed snapshot字段仍为2。PXMT只能在已经取得并strict验证相关PXMT时present；不能为填段而合成。`F|U`不再接受successor。
post-P Runtime terminal到Controller phase的映射只有四种：trusted、correlated、signed `ActiveReady`必须逐byte archive并终结`R`；`NoEffectRejected`必须逐byte archive并终结`F`；`Quarantined`是保留`MayHaveStarted`与quarantine/census事实的definitive terminal，必须逐byte archive并终结`F`，不能仅因`MayHaveStarted`改成unknown；`Uncertain`必须逐byte archive并终结`U`。这四种accepted PXMT一旦存在，PXMJ中的exact PXMT bytes、PXDM/PXDO `runtime_terminal_receipt_digest`都必须nonzero并逐byte关联，不能只保存enum或摘要。对non-empty artifact activation PXAR12，`EmptyExactZero`是invalid semantic，即使重新签名、channel/request correlation正确也不得archive或形成R/F；若transport peer、request与operation attribution仍可信但send/effect outcome未知，只能以PXMT absent、terminal digest zero的post-P `U`终结，top/owner/operation attribution失真则固定OWNER。另一个PXMT-absent合法边界只有send前已证明Runtime零effect的post-P `F`；partial send、protocol close、invalid terminal semantic或Receipt durability unknown在可信attribution下都只能是PXMT-absent `U`。PXMT-absent F/U的PXDO必须保留exact P prefix但terminal digest为zero，不得合成Receipt bytes。

五份artifact PXMT中的`resource_census_digest`、`raw_outcome_digest`以及两份Quarantined terminal使用的`quarantine_reason`只能由ledger语义facts和exact PXAR12 request digest派生，semantic ledger不得直接保存这12个32-byte输出。既有Runtime v1的三个domain与`Digest32Builder`字段顺序逐字冻结如下；实现不得换domain、换序、改用raw concatenation、跳过optional-presence discriminator或把generation absence编码成别的值：

```text
Digest32Builder("paraegox.runtime.managed-model-agent-stack-resource-census.sha256.v1")
  .field_bytes(u16_be(physical_binding_census))
  .field_u16(u16(census_complete))
  .field_u16(u16(fabric_ready))
  .field_u16(u16(model_ready))
  .field_u16(u16(agent_ready))
  .field_u16(u16(fabric_to_agent_dependency_ready))
  .field_u16(u16(model_to_agent_dependency_ready))
  .field_u64(fabric_generation_or_zero)
  .field_u64(model_generation_or_zero)
  .field_u64(agent_generation_or_zero)
  .finish()

Digest32Builder("paraegox.runtime.managed-model-agent-stack-raw-outcome.sha256.v1")
  .field_u16(raw_code)
  .field_u16(raw_context_present ? 1 : 0)
  [.field_digest(derived_quarantine_reason)]
  .field_digest(exact_PXAR12_envelope_request_digest)
  .finish()

Digest32Builder("paraegox.runtime.managed-model-agent-stack-quarantine.sha256.v1")
  .field_u16(raw_code)
  .field_u16(model_cleanup_exact_zero = none ? 0 : some_false ? 1 : 2)
  .field_digest(exact_PXAR12_envelope_request_digest)
  .finish()
```

这里每个boolean的`u16`编码严格为false=`0`、true=`1`，generation absent严格为`field_u64(0)`；raw context absent时presence为0且方括号内字段完全省略，present时presence为1且该字段恰为上一公式派生的quarantine reason。ledger的五个terminal输入唯一为：ActiveReady `raw_code=1/context=none`；NoEffectRejected只选择deadline-expired分支`raw_code=20/context=none`，明确不选择code21；Uncertain `raw_code=32/context=none`；model-intent Quarantined为`raw_code=30/context=derived_quarantine_reason/model_cleanup_exact_zero=some_true`；agent-intent Quarantined为`raw_code=31/context=derived_quarantine_reason/model_cleanup_exact_zero=some_true`。独立于production encoder的oracle必须按这三条现有公式派生actual bytes，再把结果逐byte hardcode进五份PXMT与十一份PXMA outer golden并由两侧decoder验证；production encoder只能与hardcoded结果比较，不能生成expected bytes或反向把digest写回ledger。negative必须分别交换raw code、context presence/reason与cleanup discriminator，在重算三类digest、重签PXMT并重算outer checksum后仍被phase-specific expected derivation拒绝；该测试不得以修改Runtime domain或公式获得通过。
任一非A phase必须逐 byte等于最后PXDM state；C/P无PXDO，R/F/U的latest PXDO必须等于phase。reopen必须独立重算当前header successor sequence和committed C pin：任何post-C PXDM/PXDO不是2、任何pre-C F/U不是0、R golden不是header 4/records 2，或presence/phase/high-water/chain不一致、mixed operation、old/new frame publication不明、A marker无法重建，都使整个Controller owner失败关闭。PXMJ2 frame的durable存在本身就是workspace B top-level cutover marker；不得再创建parallel marker、counter或desired file。任一未列phase byte、第四份PXDM、第二份PXDO与任一nonzero reserved superseding digest都strict reject；R0不能在PXMJ2内启用保留空间。
...
- F0 v2 phase9/10只允许tag1 activation terminal：active必须absent，pending必须present并逐byte保留exact tag1 PXAR12、response channel、admitted clock/at/deadline；terminal archive必须恰有一条matching exact PXAR12/PXMT，phase与terminal archive在同一snapshot successor durable commit，generation high-water与全部present generation逐项相等。phase9只允许从phase2 ModelStartIntent snapshot sequence10进入sequence11：Fabric7/Model8 nonzero、Agent严格absent；唯一producer是model start effect/cleanup census outcome不明、但Runtime owner/request/signer attribution仍可信时签发并保存`raw_code = 32`的exact Uncertain PXMT，census0/incomplete且全部readiness/dependency false。任何Agent-present phase9都strict reject。phase10有且只有两种Quarantined successor：model-intent分支从sequence10进入sequence11，Fabric7/Model8 nonzero、Agent absent、`raw_code = 30`；agent-intent分支从phase3 sequence11进入sequence12，Fabric7/Model8/Agent9全部nonzero、`raw_code = 31`。两种phase10都census0，model-intent为complete/Fabric-ready-only，agent-intent为incomplete/Fabric-ready-only，且quarantine reason分别严格由raw_code 30/31、model_cleanup_exact_zero=some_true与同一exact PXAR12 request digest按冻结公式派生。tag2不得进入phase9/10；missing pending/terminal、outcome/phase不符或任一request/channel/generation/clock/correlation漂移都strict reject并使Runtime owner失败关闭，绝不能由caller、digest或缺失Receipt合成Uncertain/Quarantined。transport未取得PXMT时只由Controller PXMJ U保存owner uncertainty，不构造PXMA terminal；pre-PXMA NoEffect同样不创建虚假PXMA。
- tag 2是F0唯一retire producer，只由external resident graceful `down`/joined shutdown使用。down开始时读取一次Runtime owner clock，`admitted_clock_generation`固定为该reading的generation，`admitted_at_nanos = now`，`deadline_nanos = checked_add(now, agent_drain + agent_stop + model_drain + model_stop + fabric_drain + fabric_stop)`；六个budget逐项来自retained exact PXTE11，任一overflow pre-effect拒绝。tag2只允许durable phase 5/6/7且active present；pending exact PXAR12逐byte等于`active.request`，response channel与Fabric/Model/Agent三个generation分别等于active，三个clock/deadline字段在phase5/6/7逐byte不变。Runtime必须先durable commit并exact reopen phase5 AgentRetireIntent，才执行Agent physical retire/join；再以同样顺序phase6 ModelRetireIntent→Model retire/join、phase7 FabricStopIntent→Fabric stop/join。它不产生第二signed request、新desired、deactivate request或PXTE `EmptyDeactivate`，不取得replacement/restart authority；每phase readiness、dependency-ready与physical-census shape仍逐项使用现有v1 invariant。
- 三个physical generation全部joined/stopped后，Runtime必须再durable commit并exact reopen phase1 ExactZero：active/pending均absent，physical census为0且complete=true，全部readiness/dependency-ready为false；generation high-waters、writer/revision/replay state与原ActiveReady terminal archive逐byte保留。只有这份ExactZero exact reopen后lifecycle才可发布Stopped；Controller PXMJ2保持historical R，不新增desired/PXAR/PXDM/PXDO。initial activation在Controller尚未终结时的crash可留下top P/U；但tag2 retire期间的resident或Runtime crash绝不把historical R降成P/U，top仍为R，PXMA保留最后一份durable phase5/6/7或已完成的ExactZero prefix，lifecycle固定Unknown/Failed并等待R0显式successor。F0不启动新resident恢复。

tag 0、3..255、phase 8、phase/presence不匹配、tag2 request/channel/generation/clock/deadline任一漂移全部拒绝整份snapshot。PXMA v1的one-byte tags继续保持`1 ActivateStack`、`2 DeactivateStack`、`3 RecoverActive`及其PXAR9/EmptyDeactivate语义，不能用v2名称重新解释；v1/v2 decoder按outer version双向cross-reject。缺少phase9/10所需retained pending、terminal archive或任一correlation时必须strict reject并映射Runtime owner failure，不能构造Uncertain/Quarantined、发明deactivate/recovery contract或回退D0a；R0必须以显式wire/state successor准入异常恢复。
...
该top owner的唯一filesystem layout固定为final root `<state_root>/artifact-external-controller-v1/`与同级首次publication staging `<state_root>/.artifact-external-controller-v1.initializing/`。final/staging必须是current uid/gid、mode0700、全链no-symlink的real directory；stable final entry set严格且只有zero-length `artifact-external.lock`与exact PXMJ2 `artifact-external.pxmj`，successor transaction期间只可额外出现fixed `.artifact-external.pxmj.next`。三份file都必须current uid/gid、mode0600、regular、single-link，并以no-follow/`O_CLOEXEC` descriptor复核device/inode/owner/mode/nlink/length；top root不得使用`controller.lock`、`controller.snapshot`、随机temp或其他alias。top root/staging与既有legacy `<state_root>/ctl/`、successor `<state_root>/succ/`必须互不重叠，也不得创建第二counter、journal、desired、Receipt或marker file。

首次A只在request、config、A0、materialization Receipt、fresh-workspace与全部path preflight通过并取得lifecycle gate后开始。preflight必须pin并复核同一`state_root` fd的dev/ino/uid/gid/mode，之后的staging/final mkdir/open/rename/fsync/reopen全部使用该fd上的descriptor-relative `*at`操作。`<state_root>/operator-v1/`与其`owner.lock`在此仅是non-authoritative scaffolding；D0b不能复用现有`run_up`的Starting-first顺序，任何lifecycle generation/record/socket、lower owner或Runtime mutation都必须晚于durable PXMJ2-A。staging内创建、fsync并exclusive-lock exact `artifact-external.lock`，以当次one-shot instance candidate、exact PXDQ/PXDK与admission high-water 1 no-overwrite写出`artifact-external.pxmj`；lock与PXMJ逐文件fsync、close/exact reopen并复核后fsync/reopen staging directory。rename前再次复核pinned state-root fd且config path仍解析到同一identity，只在该fd上用whole-directory `RENAME_NOREPLACE`发布final，fsync/reopen同一parent fd，再只从该fd exact reopen final root，并从reopened root重开复核lock与PXMJ。state-root path replacement、final child swap或任一descriptor drift固定OWNER/null，不能声称publication durable；完整publication才是首次authoritative lifecycle/deployment mutation，之后才可发布D0b lifecycle generation/record或建立lower owner。

final与staging都absent才是virgin，deployment query固定NotFound/`changed = false`且不创建root、staging或lock；final root绝不接受empty、lock-only或“尚未初始化”状态。same deployment operation + exact same PXDQ的strict-valid staging可由deploy在其exclusive staging lock内完成上述publication；read-only query只能strict读取并返回`PXLC-DEPLOY-UNCERTAIN`、`changed = false`，绝不publish。different/cross-operation、partial/unknown staging、final+staging或final存在但缺少valid exact A snapshot（包括empty/lock-only）全部固定`PXLC-DEPLOY-OWNER`。只要same staging、final或ambiguous candidate存在，就必须先strict reopen并沿用其中的exact controller store instance，绝不能先redraw；只有严格证明A candidate与final/staging全部absent，下一次尝试才可重新draw。A publication的old/new或durability仍不能唯一判定时固定`PXLC-DEPLOY-UNCERTAIN`/`changed = null`，不得猜A、签U或创建第二root。

A之后每份PXMJ2 successor只执行fixed-next protocol：在exclusive top lock内先strict reopen active root/lock/snapshot，并收口attributable same-operation `.artifact-external.pxmj.next`；新next只能以`O_EXCL | O_NOFOLLOW | O_CLOEXEC`/0600创建，write-all、fsync、close、exact reopen并复验canonical bytes。replace前再次复核root/lock/active snapshot identity与原sequence未变，只允许header sequence精确`N + 1`且属于Program冻结的A→C/P/R/F/U successor；随后atomic replace `artifact-external.pxmj`、fsync top root并exact reopen final snapshot。mutator只可收口same-operation canonical next；query绝不rename、fsync、cleanup、recover或settle next。valid attributable same-operation old/new publication或durability仍不唯一时固定`PXLC-DEPLOY-UNCERTAIN`/`changed = null`；invalid/mixed/cross-operation/extra entry、identity、codec/checksum或committed-head contradiction固定`PXLC-DEPLOY-OWNER`并使用已冻结changed三态。unknown next永不扫描推理或删除。

mutating owner的全局lock acquisition顺序严格为resident-lifetime lifecycle `<state_root>/operator-v1/owner.lock` → transaction-scoped top `artifact-external-controller-v1/artifact-external.lock` → legacy Controller `<state_root>/ctl/controller.lock` → successor `<state_root>/succ/managed-fabric.lock`，只允许该顺序或其前缀；Controller绝不取得Runtime store lock。成功取得successor并strict重验cutover marker与active predecessor后，允许现有lower API立即显式drop legacy lock而继续保持successor到本phase top successor durable+exact reopen，随后drop successor再drop top；在取得successor前或任一failure path则按实际已持有集合逆序释放，绝不为满足表面顺序重取已drop lock。lifecycle lock只在joined shutdown时由resident最后释放。A在resident取得lifecycle owner lock之后、任何Starting record/socket/lower effect之前只取得top；C/P/R/F/U的每个phase在已持resident lifecycle lock下重新取得top exclusive与需要的legacy/successor lock，strict validate/commit/reopen后按上述现有API语义释放lower与top，不跨phase长期持有Controller locks。C必须在同一top-lock transaction内把之后public query所需的全部facts封存进self-contained PXMJ2。P successor durable且exact reopen后，必须drop全部top/legacy/successor lock再做transport，resident lifecycle owner lock仍保持；收到PXMT后在该resident gate下重新按top→legacy→successor取得并重验全部prefix，才可提交R/F/U。D0a与D0b mutation由同一个resident lifecycle owner串行：旧D0a不得创建或取得不存在的top lock，也不得创建任何top scaffolding；它在已持lifecycle gate内strict确认top final、staging与next都absent，并由resident持续持有该gate穿过legacy/lower transaction直到joined shutdown。任一top evidence按上文D0a cutover规则返回且不进入lower；任何D0a/D0b/restart路径都禁止lower→top。

D0b首次mutating resident对strict-valid既有`owner.lock`只做一次nonblocking exclusive acquisition；若返回`WouldBlock`/`EWOULDBLOCK`，必须立即终结`PXLC-DEPLOY-UNCERTAIN`/exit1、`changed = false`，不能wait、follow winner、retry、读取top partial/staging、进入lower、创建Starting record/socket或启动任一owner/workload。preflight total order保持不变：只有在gate前已经strict完成的PF、O、MR才可原样保留，未完成项为null；exact input deployment operation id始终回显，`state = "uncertain"`、`terminal_outcome = "uncertain"`，generation、revision、controller committed pin、DR与两个Runtime digest全部null，`current_health_checked = false`且diagnostics恰一项。winner结束后只能由一个新的CLI invocation以same id执行正常replay/query；原contender不得在本调用内自动跟随、重试或接管。该首次gate contention发生在任何D0b owner mutation前，因此不使用后续phase的累计changed=true/null分支。

public deployment query按final-root存在性执行两段式、TOCTOU-safe bootstrap selection，且绝不创建operator directory、owner lock或top root。若从pinned state root首次no-follow检查见final root存在，它直接一次try shared top lock并strict读取self-contained PXMJ2，绝不取得lifecycle/lower/Runtime lock。若首次见final absent，它只可strict open既有`<state_root>/operator-v1/owner.lock`并一次try shared：operator root与owner.lock都absent时，必须从同一pinned state root重查final、staging、operator root与lock仍全部absent，才返回NotFound/`changed = false`；此时任一staging存在而gate/lock absent固定`PXLC-DEPLOY-OWNER`。operator root或lock metadata/layout/identity strict-invalid固定OWNER；owner.lock在两次观察间出现、消失或无法稳定证明时固定`PXLC-DEPLOY-UNCERTAIN`/false，且不读取partial staging。既有strict-valid owner lock `WouldBlock`同样固定UNCERTAIN/false并且不得读staging；取得shared gate后才从pinned state root重查final/staging，final出现则保持lifecycle→top顺序再try shared top并strict读取，双absent为NotFound，same-operation strict-valid A staging为UNCERTAIN/false，invalid/cross/partial staging或final+staging为OWNER。Running final query因此不被resident长期持有的lifecycle gate阻塞；任何root selection后都必须在所选descriptor链上再验证identity/entry set，不能把初次路径观察或ENOENT当authority。

bootstrap lifecycle `WouldBlock`与strict-valid top lock `WouldBlock`的exact envelope均固定`ok = false`、`changed = false`、回显strict input operation id、`state = "uncertain"`，其他projection全部null，恰一项`PXLC-DEPLOY-UNCERTAIN`、exit 1。hardcoded `artifact_f0_deployment_query_d0a_resident_contended_v1.json`使用ledger deployment operation id，固定为下列单行canonical compact JSON加一个LF；它覆盖top absent但D0a resident正exclusive持有owner.lock的workspace，证明不能误报NotFound或读取staging：

```json
{"schema_version":1,"command":"deployment.operation.query","mode":"local","ok":false,"changed":false,"operation_id":"d1d1d1d1d1d1d1d1d1d1d1d1d1d1d1d1","state":"uncertain","profile":null,"artifact_object_ref":null,"materialization_receipt_ref":null,"generation":null,"deployment_revision":null,"controller_snapshot_sequence":null,"deployment_receipt_ref":null,"runtime_apply_request_digest":null,"runtime_terminal_receipt_digest":null,"terminal_outcome":null,"current_health_checked":false,"diagnostics":[{"code":"PXLC-DEPLOY-UNCERTAIN","message":"deployment operation outcome is uncertain"}]}
```

mutating contention的独立hardcoded oracle固定为`tests/fixtures/wire/artifact_f0_deploy_resident_contended_v1.json`：它按上述19-key顺序保存一个compact object加单一LF，`command = "deploy"`，回显ledger deployment operation id，PF/O/MR逐byte取同一ledger驱动的hardcoded profile与`artifact_f0_text_refs_v1.txt`前两行，其他字段严格按上段为null/uncertain/false，且只有同一UNCERTAIN diagnostic。fixture禁止调用production serializer生成expected bytes；另一个focused null-projection case在PF/O/MR尚未strict完成的人工gate seam证明三项必须null，不能从input text或winner状态猜值。

query投影的是historical Controller outcome且`current_health_checked = false`，不读取或验证当前lower/Runtime，不走owner IPC，也不fsync、cleanup、recover或settle next；bootstrap lifecycle lock仅串行A root选择，取得后也不得触发任何mutation。top-only public query只验证PXMJ canonical codec/checksum、nested PXAR/PXMT canonical digest与全部cross-frame correlation；它信任Controller owner在C/R commit前已经于ordered lower context用可信public keys完成Controller与Runtime两份Ed25519验证，不读取identity/lower/Runtime或独立验证Runtime Receipt签名。mutating reopen/replay仍必须在ordered lower scope以可信keys重新验证两份签名。signature byte drift若未同步nested digest/chain/PXMJ checksum必须映射OWNER；测试不得把该结构性拒绝宣称成query独立crypto verify，也不得为此新增key bundle或改变PXMJ layout。mutator任一phase的top或后级lock `WouldBlock`固定`PXLC-DEPLOY-UNCERTAIN`，但`changed`按本invocation累计owner effect计算：至此零mutation可证才为false；至少一份initial whole-directory或canonical successor已完整publish/fsync/exact reopen且之后零additional effect/ambiguity时为true；任一先前或当前effect/durability不明时为null。只保留preflight已验证的PF/O/MR。read-only query的`WouldBlock`始终false。其他lock syscall failure或lock/root/file metadata/layout/identity invalid固定`PXLC-DEPLOY-OWNER`，effect前为false，之后只按已冻结true/null规则；所有lock acquisition都nonblocking、一次、无wait/retry。

所有deploy/query/replay的success、error、NotFound、lock、owner与output路径必须先把public projection复制成不持fd/guard/borrow的owned value，再按实际仍持有集合显式unlock/drop successor、legacy（若现有lower API尚未早释）、top、bootstrap-shared lifecycle及全部CLI clone/store handle，之后才允许JSON serialize、stdout write与flush。resident supervisor自己的exclusive lifecycle owner lock独立保持到joined shutdown；CLI不得继承或drop该owner handle，也不得持有其clone。fork/spawn/exec前必须drop全部非owner clone，child意外继承时防御性unlock/close。blocked/failing stdout不得持锁、重锁、cleanup、rollback或改变durable bytes与`changed`归因。

durable A exact reopen之后、C之前的effect顺序与归因必须完整：先是`identity::load_or_create`的profile-root lock/identity directory/`.writer.lock`/manifest temp与final及其file/parent fsync/reopen，再是`layout::prepare`的`ctl`、`succ`、`auth`、`rt`、`node`、store、bootstrap与socket directories及逐层mkdir/fsync/reopen，再bind/chmod/fsync control socket并publish/fsync/reopen Starting record，最后才可进入lower/Runtime。上述每一项都严格晚于A且计入本invocation累计effect；不得把identity/layout/lifecycle bootstrap称为zero-effect scaffolding。

pre-C `F`只要求没有desired/Runtime activation effect，且上述已发生的identity/layout/Starting/lower准备effect及其durability全部可确定、failure outcome确定、top仍可可靠commit；它必须保留Starting record已durable时的exact generation。本调用新发布A、任一identity/layout/lifecycle effect或F successor时累计`changed = true`，只有既有F的pure terminal replay为false。任一identity temp/rename/parent fsync、layout mkdir/fsync/reopen、record temp/publish、socket bind/chmod/parent-fsync/cleanup或lower/Runtime effect outcome/durability不明，但top/lower owner identity与operation attribution仍可信时，只能由可靠top提交保留verified prefix的pre-C `U`，committed snapshot pin为0。`changed`只度量canonical owner-state mutation：本调用可靠publish/fsync/exact-reopen terminal U successor固定true，pure U replay为false；U/snapshot publication ambiguous或record不足才为null，U所封存的attributable lower effect unknown不否定已证明的owner commit。top/lower owner identity、operation attribution、codec/head integrity失真或top不能可靠记录时固定OWNER/null且不得签U。同样规则逐项适用于C/P后F/U。top A或任一N+1 publication本身old/new仍不明固定UNCERTAIN/null，不能猜U；任何phase都按本invocation从A起的累计false/true/null归因，不能因“本phase零effect”把先前durableA/C/P错误降为false。

唯一supervisor必须按上述每phase transaction-scoped acquisition顺序遵守固定effect序列，而不是跨phase或transport同时长期持有top/lower locks：包含exact PXDQ/PXDK与admission high-water 1的canonical PXMJ2-A frame在同一首次durable commit发布；该frame的存在是workspace B第一次lifecycle/deployment mutation与唯一cutover marker，marker digest按上述immutable四元组公式派生而非使用mutable frame checksum；之后才可初始化lower Authority/Runtime并建立managed-Fabric predecessor；exact active Fabric predecessor确认后，Controller按上述一次64-byte CSPRNG draw构造fresh Runtime ApplyOperationId/TemporalConstraintId/auth nonce与signed PXAR12，并在一次top-level PXMJ2-C durable successor内原子写入PlanContent v2、PlanContentDigestV2、PXTE11、successor revision、该exact PXAR12与PXDM-C；transport前先以同一PXAR的request digest durable PXMJ2-P/PXDM-P并exact reopen，并按现有lower API早释legacy、随后drop successor与top，确保全部Controller/lower lock已释放；只在收到PXMT1后重新按同序取得locks、重验prefix并strict验证同一Runtime operation，才提交PXMJ2-R、PXDM-R与PXDO-R/Receipt high-water。lower/Runtime确定失败或effect不明只在top仍可靠时按已验证prefix终结F/U；top publication ambiguity返回UNCERTAIN，top owner失真返回OWNER，不回滚/重写已durable facts，也不调用D0a。

PXMJ v2 marker一旦durable存在，必须先按上述strict codec/identity规则取得可信PXDQ config commitment；同一workspace中所有D0a或旧managed-stack入口在commitment匹配时只能返回replace-required，可信mismatch优先返回lifecycle configuration，无法可信读取则返回各入口既有owner/lifecycle-reconcile分类。lower store不能在supervisor重启、top-level failure或output loss后独立重开成为第二desired writer。只有mutating deploy/restart的PXMJ2 reopen必须在上述同一ordered lock scope strict reopen lower predecessor与全部top-level bytes；public query只读self-contained PXMJ2且绝不打开lower。缺失、版本混合、head冲突或无法证明old/new commit都按上述UNCERTAIN/OWNER边界失败关闭，不从lower store反推top-level success。这样PXMJ2是唯一desired head，PXMA2仍只是Runtime live/recovery owner，不形成Controller双写。

#### Artifact cutover 对 legacy lifecycle 入口的约束

任何将要启动compiled legacy stack的D0a、foreground `chat`与hidden legacy supervisor，都必须在config/Secret/request preflight后、Starting record/socket与任一legacy/lower effect前，由resident exclusive lifecycle owner gate串行并strict重查top final、staging与canonical next。它们必须先验证owner/codec到足以可信读取PXDQ immutable config commitment：strict-valid final或same-operation staging/next的commitment与current config不等时，`PXLC-LIFECYCLE-CONFIGURATION`优先于replace、reconcile与operation lookup，exit 2且零effect；只有commitment匹配的strict-valid PXMJ2 final或可归因完整A staging/canonical next才返回`PXLC-DEPLOY-REPLACE-REQUIRED`/`existing deployment state requires explicit replacement`、exit 2。invalid/partial/mixed/cross-operation、identity不明或无法可信读取commitment的evidence对D0a使用其既有`PXLC-DEPLOY-LIFECYCLE`，对`up`/`chat`使用既有`PXLC-LIFECYCLE-RECONCILE-REQUIRED`/`managed-local lifecycle authority is uncertain and requires explicit recovery`、exit 1；不得从corrupt raw offset猜config。D0b deploy/query同样先strict建立可信commitment：可信mismatch使用configuration，无法可信解码则使用DEPLOY-OWNER。hidden supervisor只有在自己赢得resident lease并即将创建generation时才执行race-closing recheck，并把typed configuration/replace/reconcile结果原样传给caller，不能退化成generic startup failure、跟随external winner声称compiled success或直接打开lower。

plain M2a `up`仍是generic lifecycle ensure，不声明compiled profile或D0b Receipt：same-config external owner已经Starting/Running，或本child因owner contention输给D0b winner而winner在既有bounded wait内到达Running时，沿用原M2a 9-key成功投影、同generation、`changed = false`且零lower effect，不把marker本身当成start authorization。只有would-be start在NeverStarted/Stopped/Failed或无live owner下赢得resident lease时才执行上述top recheck；commitment匹配的valid marker固定replace-required，可信mismatch固定configuration，绝不启动compiled fallback或未授权external restart。concurrent winner未到Running、退出或只留下A时只能返回原unknown/unavailable，owner释放后重见matching valid marker则replace-required，不得接管。D0a deploy与foreground chat声明compiled behavior，因此在任何same-config valid marker/winner下始终replace-required，而不能使用plain-up external Running fast path；different-config可信marker始终按configuration优先级失败。

foreground `chat`在config/Secret preflight后先做strict final quick-read；只有owner/codec可信、PXDQ commitment匹配的valid final才立即replace-required，可信mismatch立即返回上文exact configuration stderr/exit2。quick-read不能稳定分类或commitment不可信时不得读partial staging或猜config，随后只nonblocking取得一次resident lifecycle gate：contention沿用`PXLC-LIFECYCLE-UNAVAILABLE`，取得后再strict重查final/staging/next并按configuration→replace→reconcile优先级分类，且正常absent启动后持有exclusive gate直到joined shutdown。replace时chat stdout严格为空、stderr恰为`paraegox: code=PXLC-DEPLOY-REPLACE-REQUIRED message=existing deployment state requires explicit replacement\n`、exit 2且不追加usage；reconcile同样stdout为空，并使用既有code/message的一行stderr与exit 1。正常absent路径的grammar、presentation、child handoff与joined-shutdown语义保持原样。

`status`继续只读lifecycle record/owner，不读取PXMJ2或当前lower/Runtime：A-only/full staging但尚无generation且无live resident时仍是原`never_started`/`changed = false`，external Starting/Running/terminal record按同一generation原样投影。`down`只经live resident对external Running执行原反向joined shutdown，绝不删除/改写PXMJ2、签新Controller desired/Receipt、调用D0a deactivate或启动legacy；A-only crash且无resident仍是原`never_started`/false，已joined external state保留原terminal lifecycle record与immutable historical PXMJ2-R。`tui`、Inspection与receipt命令只按既有lifecycle规则attach同config external Running generation，不能仅因marker拒绝；非Running（包括down后）仍使用各自既有not-running路径且绝不start。

control unavailable且disk观察为NeverStarted/Stopped或pre-record候选时，`status`/`down`不得直接返回成功：只strict open既有owner.lock、一次nonblocking shared检查，绝不创建或retry。lock held表示A→Starting或其他active transition，固定原9-key相应command、`ok = false`、`state = "unknown"`、`changed = false`、`inspection_checked = false`、恰一项`PXLC-LIFECYCLE-UNAVAILABLE`/`managed-local lifecycle owner is unavailable`、exit 1；fresh A窗口的generation/readiness为null/false，已有strict record时只可保留其generation/readiness。lock free后必须在gate内重查record/socket/top，稳定无live owner才可返回NeverStarted/Stopped；lock/root absent也从pinned state root二次核验，appearance/disappearance固定同一unknown/unavailable。所有路径一次、无wait/retry，且不删除marker。

hardcoded cutover fixtures全部保持M2a原9-key顺序。`artifact_f0_up_cutover_a_only_v1.json`、`artifact_f0_up_cutover_post_down_v1.json`、`artifact_f0_up_cutover_reconcile_v1.json`分别固定为下列单行compact JSON加一个LF；post-down使用ledger lifecycle generation并保留已观察readiness：

```json
{"schema_version":1,"command":"up","ok":false,"state":"unknown","generation":null,"changed":false,"owner_readiness_observed":false,"inspection_checked":false,"diagnostics":[{"code":"PXLC-DEPLOY-REPLACE-REQUIRED","message":"existing deployment state requires explicit replacement"}]}
```

```json
{"schema_version":1,"command":"up","ok":false,"state":"stopped","generation":"d2d2d2d2d2d2d2d2d2d2d2d2d2d2d2d2","changed":false,"owner_readiness_observed":true,"inspection_checked":false,"diagnostics":[{"code":"PXLC-DEPLOY-REPLACE-REQUIRED","message":"existing deployment state requires explicit replacement"}]}
```

```json
{"schema_version":1,"command":"up","ok":false,"state":"unknown","generation":null,"changed":false,"owner_readiness_observed":false,"inspection_checked":false,"diagnostics":[{"code":"PXLC-LIFECYCLE-RECONCILE-REQUIRED","message":"managed-local lifecycle authority is uncertain and requires explicit recovery"}]}
```

`artifact_f0_status_a_transition_unavailable_v1.json`与`artifact_f0_down_a_transition_unavailable_v1.json`只在`command`不同，exact bytes分别为：

```json
{"schema_version":1,"command":"status","ok":false,"state":"unknown","generation":null,"changed":false,"owner_readiness_observed":false,"inspection_checked":false,"diagnostics":[{"code":"PXLC-LIFECYCLE-UNAVAILABLE","message":"managed-local lifecycle owner is unavailable"}]}
```

```json
{"schema_version":1,"command":"down","ok":false,"state":"unknown","generation":null,"changed":false,"owner_readiness_observed":false,"inspection_checked":false,"diagnostics":[{"code":"PXLC-LIFECYCLE-UNAVAILABLE","message":"managed-local lifecycle owner is unavailable"}]}
```

`artifact_f0_chat_cutover_replace_required_v1.stderr`与`artifact_f0_chat_cutover_reconcile_required_v1.stderr`各为下列单行UTF-8 bytes加恰一个LF；对应stdout fixture为空文件：

```text
paraegox: code=PXLC-DEPLOY-REPLACE-REQUIRED message=existing deployment state requires explicit replacement
```

```text
paraegox: code=PXLC-LIFECYCLE-RECONCILE-REQUIRED message=managed-local lifecycle authority is uncertain and requires explicit recovery
```

config-precedence fixtures分别把strict-valid final、same-operation A staging与canonical next中的可信PXDQ commitment替换成另一份nonzero current authority，三种都必须在replace/reconcile/operation lookup前命中`PXLC-LIFECYCLE-CONFIGURATION`；再分别叠加checksum/identity/codec corruption，证明commitment不再可信时D0b/query归DEPLOY-OWNER、D0a归既有DEPLOY-LIFECYCLE、up/chat归既有LIFECYCLE-RECONCILE-REQUIRED，绝不从raw offset猜configuration。`artifact_f0_d0a_cutover_configuration_v1.json`使用上文14-key exact bytes；`artifact_f0_up_cutover_configuration_v1.json`固定为下列M2a原9-key单行compact JSON加一个LF：

```json
{"schema_version":1,"command":"up","ok":false,"state":"unknown","generation":null,"changed":false,"owner_readiness_observed":false,"inspection_checked":false,"diagnostics":[{"code":"PXLC-LIFECYCLE-CONFIGURATION","message":"managed-local lifecycle configuration authority changed"}]}
```

`artifact_f0_chat_cutover_configuration_v1.stderr`固定为下列UTF-8 bytes加恰一个LF，对应stdout为空、exit 2且不追加usage：

```text
paraegox: code=PXLC-LIFECYCLE-CONFIGURATION message=managed-local lifecycle configuration authority changed
```

D0b deploy/query的different-config final/staging/next各自使用hardcoded完整public envelope，保留已strict parse的input operation id并把state/profile/O/MR/generation/revision/controller snapshot/DR/Runtime digests/terminal outcome全置null，`changed = false`、恰一项同一configuration diagnostic、exit 2；source guard证明该分支先于replace、operation lookup和DEPLOY-OWNER，但只在owner/codec已可信时可达。

#### Mandatory lower-store fence

`developer_fixture_agent_stack.rs`中四个legacy run facade `run_developer_fixture_agent_stack_v1`、`run_developer_provisioned_agent_stack_v1`、`run_developer_fixture_model_agent_stack_v1`、`run_developer_provisioned_model_agent_stack_v1`与三个deactivate facade `deactivate_developer_fixture_agent_stack_v1`、`deactivate_developer_fixture_model_agent_stack_v1`、`deactivate_developer_provisioned_model_agent_stack_v1`不得继续作为可绕过top的raw public入口，也不得由`lib.rs`直接re-export。它们的lower open/resume mechanics收敛到唯一mandatory checked facade；每次调用input都必须携不可省略、不可伪造或Clone的resident lifecycle owner lease与typed top disposition。legacy run/deactivate disposition只有在同一resident gate覆盖整个调用、strict证明top final/staging/next全部absent时才能构造；任何raw facade、default/optional proof、调用点自行布尔值或先check后release都禁止。

D0b另用predecessor-only checked facade：它只在exact PXMJ2-A/C prefix与top exclusive transaction lease下建立或strict验证Fabric predecessor，不得调用`advance_model_agent_active`、构造PXAR9、进入任一legacy model/agent run facade或调用deactivate。source/dependency guard必须枚举仓内每个`open_or_initialize_legacy`与`ManagedFabricSuccessorStoreV1::resume_from_cutover_marker_developer_local` call site，证明DeveloperLocal root不存在unfenced lower入口；distributed/G2使用不同root的明确call site才可作为逐项列名豁免，不能用模块级通配。system harness必须从external R snapshot/digest开始，证明Running期间plain up replay同generation/false且top不变，down只joined-stop、不调用D0a deactivate且不修改Controller desired/PXMJ history，随后historical deployment query仍返回同一R且`current_health_checked = false`，up/chat拒绝、tui走not-running，ArtifactStore与top bytes逐byte不变。

新增 successor hardcoded shared goldens 固定为 `artifact_f0_binding_v1.hex`（192 bytes）、`artifact_f0_plan_content_v2.hex`、`artifact_f0_pxte_v11.hex`、`artifact_f0_runtime_slice_v11.hex`（exact PXTA-zero || PXTE11）、`artifact_f0_pxar_v12.hex`、ActiveReady的`artifact_f0_pxmt_artifact_v1.hex`、`artifact_f0_pxmt_artifact_no_effect_rejected_v1.hex`、`artifact_f0_pxmt_artifact_uncertain_v1.hex`、model-intent Quarantined的`artifact_f0_pxmt_artifact_quarantined_v1.hex`、agent-intent Quarantined的`artifact_f0_pxmt_artifact_quarantined_after_agent_intent_v1.hex`，以及本批首次建立的predecessor state fixture `artifact_f0_pxmj_v1.hex`与`artifact_f0_pxma_v1.hex`；v2每个可达normal phase使用下文逐phase名称，禁止另设`artifact_f0_pxmj_v2.hex`或`artifact_f0_pxma_v2.hex` alias形成第二oracle。每份`.hex`都只能是单行lower-case hex加一个LF。五份PXMT必须分别由runtime test seed重签并由独立decoder验证完整canonical/correlation/signature语义，不能改写ActiveReady fixture或只mutation outcome byte。所有wire/state golden的测试常量只能来自单一shared semantic ledger `tests/fixtures/wire/artifact_f0_semantic_ledger_v1.json`，其他fixture不得复制一份可独立漂移的常量表。ledger的top-level key顺序精确为`format,payload_utf8,artifact_store,deployment,runtime_apply,authority,signing,predecessor,agent_plan,model_plan,runtime_terminal,controller_shapes,runtime_state,text`；内容必须是下列单行canonical compact JSON加一个LF，不能pretty-print、重排key、转义ASCII、追加空白或第二个换行。`artifact_store.snapshot_frame_bytes`只表示PXAZ frame length；`artifact_store.accounted_logical_bytes`逐项映射该snapshot header的`accounted_rest_bytes`，二者不得混同：

```json
{"format":"paraegox-artifact-f0-semantic-ledger-v1","payload_utf8":"artifact-f0-prefix: ","artifact_store":{"store_instance_hex":"a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0","config_commitment_hex":"a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1","primary_operation_id_hex":"a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2","second_operation_id_hex":"a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3a3","primary_operation_sequence":1,"second_operation_sequence":2,"object_sequence":1,"failed_materializing_presence":{"before_pxmu":false,"after_pxmu":true},"failed_shapes":{"before_pxmu":"flags2_6_bit0_0_no_pxmu_no_remnant","after_pxmu":"flags3_7_bit0_0_pxmu_no_remnant"},"uncertain_materializing_present":true,"uncertain_object_terminal_present":false,"snapshot_sequences":{"admitted":1,"materializing":2,"object_terminal":3,"materialized_terminal":4,"materialized_receipt":5,"failed_terminal":2,"failed_receipt":3,"failed_after_materializing_terminal":3,"failed_after_materializing_receipt":4,"already_materialized_materializing":7,"already_materialized_terminal":8,"already_materialized_receipt":9,"uncertain_blocked":3,"uncertain_receipt_blocked":4},"snapshot_frame_bytes":{"admitted":672,"materializing":912,"object_terminal":1104,"materialized_terminal":1408,"materialized_receipt":1648,"failed_terminal":976,"failed_receipt":1216,"failed_after_materializing_terminal":1216,"failed_after_materializing_receipt":1456,"already_materialized_materializing":2304,"already_materialized_terminal":2608,"already_materialized_receipt":2848,"uncertain_blocked":1216,"uncertain_receipt_blocked":1456},"accounted_logical_bytes":{"admitted":672,"materializing":912,"object_terminal":1330,"materialized_terminal":1634,"materialized_receipt":1874,"failed_terminal":976,"failed_receipt":1216,"failed_after_materializing_terminal":1216,"failed_after_materializing_receipt":1456,"already_materialized_materializing":2530,"already_materialized_terminal":2834,"already_materialized_receipt":3074,"uncertain_blocked":1216,"uncertain_receipt_blocked":1456}},"deployment":{"controller_store_instance_hex":"d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0d0","operation_id_hex":"d1d1d1d1d1d1d1d1d1d1d1d1d1d1d1d1","lifecycle_generation_hex":"d2d2d2d2d2d2d2d2d2d2d2d2d2d2d2d2","admission_sequence":1,"deployment_revision":1,"receipt_sequence":1,"controller_committed_snapshot_sequence":2},"runtime_apply":{"artifact_operation_id_hex":"d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4d4","artifact_temporal_constraint_id_hex":"d5d5d5d5d5d5d5d5d5d5d5d5d5d5d5d5","artifact_authentication_nonce_hex":"d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6","legacy_operation_id_hex":"6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d6d","legacy_temporal_constraint_id_hex":"6e6e6e6e6e6e6e6e6e6e6e6e6e6e6e6e","legacy_authentication_nonce_hex":"6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f6f","clock_domain_hex":"0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a","clock_generation":3,"original_budget_nanos":6000083,"remaining_budget_nanos":6000083,"runtime_store_instance_hex":"4444444444444444444444444444444444444444444444444444444444444444"},"authority":{"source_scope_hex":"01010101010101010101010101010101","source_plan_ref_hex":"02020202020202020202020202020202","writer_hex":"09090909090909090909090909090909","writer_principal_hex":"09090909090909090909090909090909","writer_epoch":1,"tenure_authority_hex":"07070707070707070707070707070707","tenure_key_ref_hex":"08080808080808080808080808080808","tenure_algorithm":1,"tenure_algorithm_version":1,"tenure_supersedes_through_epoch":0,"tenure_nonce_utf8":"test-only-tenure-nonce","request_principal_hex":"09090909090909090909090909090909","request_key_ref_hex":"0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c","request_algorithm":1,"request_algorithm_version":1},"signing":{"test_only":true,"tenure_seed_hex":"1111111111111111111111111111111111111111111111111111111111111111","tenure_public_key_hex":"d04ab232742bb4ab3a1368bd4615e4e6d0224ab71a016baf8520a332c9778737","controller_seed_hex":"2222222222222222222222222222222222222222222222222222222222222222","controller_public_key_hex":"a09aa5f47a6759802ff955f8dc2d2a14a5c99d23be97f864127ff9383455a4f0","runtime_seed_hex":"7777777777777777777777777777777777777777777777777777777777777777","runtime_public_key_hex":"c853ad0f0cd2b619aea92ceec4fd56a24d6499d584ce79257e45cfd8139b60a7"},"predecessor":{"target_hex":"05050505050505050505050505050505","manifest_digest_hex":"fad22cd7f146653019a6b9570d06c222a34689d5b669481cdb7b314ec05edf53","build_instance_id_hex":"1111111111111111111111111111111111111111111111111111111111111111","build_descriptor_digest_hex":"29e532abc1ac2f6ea13b45ce7029020e2863e1d302c5cdab0dab0e272652a2c1","runtime_artifact_sha256_hex":"2222222222222222222222222222222222222222222222222222222222222222","active_target_slice_digest_hex":"2044e64adb9e0744f0334b8dc0ee816b1d2f857f655a7a576c9b61cf1efe51da","legacy_cutover_marker_digest_hex":"c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0c0","legacy_source_revision":3,"legacy_successor_revision":4,"fabric_service_id_hex":"51515151515151515151515151515151","fabric_prepare_budget_nanos":1000000000,"fabric_start_budget_nanos":2000000000,"fabric_readiness_budget_nanos":3000000000,"fabric_drain_budget_nanos":4000000000,"fabric_stop_budget_nanos":5000000000,"fabric_listen_endpoint":"tcp/127.0.0.1:7447"},"agent_plan":{"service_id_hex":"65656565656565656565656565656565","prepare_budget_nanos":1000000,"start_budget_nanos":2000000,"readiness_budget_nanos":3000000,"drain_budget_nanos":4000000,"stop_budget_nanos":5000000,"max_sessions":16,"max_turns_per_session":64,"max_requests_per_session":64,"max_event_batch":64,"submit_binding_id_hex":"61616161616161616161616161616161","control_binding_id_hex":"62626262626262626262626262626262","submit_key_expression":"paraegox/agent/v1/submit","control_key_expression":"paraegox/agent/v1/control","max_items":8,"max_bytes":262144,"max_frame_bytes":65536,"max_response_body_bytes":65536,"handler_timeout_nanos":5000000000,"provider_profile":2,"provider_ref_hex":"63636363636363636363636363636363","provider_config_digest_hex":"6464646464646464646464646464646464646464646464646464646464646464","provider_secret_present":false},"model_plan":{"service_id_hex":"89898989898989898989898989898989","prepare_budget_nanos":23,"start_budget_nanos":29,"readiness_budget_nanos":31,"drain_budget_nanos":37,"stop_budget_nanos":41,"max_in_flight":8,"artifact_adapter_id":"px-art-prefix-v1","legacy_adapter_id":"px-fixture-echo1","adapter_version":1,"capability_id":"px-bounded-text1"},"runtime_terminal":{"common":{"runtime_peer_hex":"71717171717171717171717171717171","local_endpoint_identity_digest_hex":"7272727272727272727272727272727272727272727272727272727272727272","peer_credentials_digest_hex":"7373737373737373737373737373737373737373737373737373737373737373","response_key_ref_hex":"76767676767676767676767676767676","response_algorithm":1,"response_algorithm_version":1,"completion_runtime_host_epoch":9,"selection_clock_generation":3},"variants":{"active_ready":{"outcome":"active_ready","lifecycle_effect":"may_have_started","head":"committed_incoming","fabric_generation":7,"model_generation":8,"agent_generation":9,"physical_binding_census":2,"census_complete":true,"fabric_ready":true,"model_ready":true,"agent_ready":true,"fabric_to_agent_dependency_ready":true,"model_to_agent_dependency_ready":true,"exact_zero":false,"quarantined":false,"raw_code":1,"raw_context":"none","completion_snapshot_sequence":12,"selection_observed_at_nanos":13},"no_effect_rejected":{"outcome":"no_effect_rejected","lifecycle_effect":"proven_not_started","head":"preserved_none","fabric_generation":7,"model_generation":null,"agent_generation":null,"physical_binding_census":0,"census_complete":true,"fabric_ready":true,"model_ready":false,"agent_ready":false,"fabric_to_agent_dependency_ready":false,"model_to_agent_dependency_ready":false,"exact_zero":false,"quarantined":false,"selection_branch":"deadline_expired","raw_code":20,"raw_context":"none","completion_snapshot_sequence":1,"selection_observed_at_nanos":21},"uncertain":{"outcome":"uncertain","lifecycle_effect":"may_have_started","head":"committed_incoming","fabric_generation":7,"model_generation":8,"agent_generation":null,"physical_binding_census":0,"census_complete":false,"fabric_ready":false,"model_ready":false,"agent_ready":false,"fabric_to_agent_dependency_ready":false,"model_to_agent_dependency_ready":false,"exact_zero":false,"quarantined":false,"raw_code":32,"raw_context":"none","completion_snapshot_sequence":11,"selection_observed_at_nanos":22},"quarantined":{"outcome":"quarantined","lifecycle_effect":"may_have_started","head":"committed_incoming","fabric_generation":7,"model_generation":8,"agent_generation":null,"physical_binding_census":0,"census_complete":true,"fabric_ready":true,"model_ready":false,"agent_ready":false,"fabric_to_agent_dependency_ready":false,"model_to_agent_dependency_ready":false,"exact_zero":false,"quarantined":true,"raw_code":30,"raw_context":"derived_quarantine_reason","model_cleanup_exact_zero":"some_true","completion_snapshot_sequence":11,"selection_observed_at_nanos":23},"quarantined_after_agent_intent":{"outcome":"quarantined","lifecycle_effect":"may_have_started","head":"committed_incoming","fabric_generation":7,"model_generation":8,"agent_generation":9,"physical_binding_census":0,"census_complete":false,"fabric_ready":true,"model_ready":false,"agent_ready":false,"fabric_to_agent_dependency_ready":false,"model_to_agent_dependency_ready":false,"exact_zero":false,"quarantined":true,"raw_code":31,"raw_context":"derived_quarantine_reason","model_cleanup_exact_zero":"some_true","completion_snapshot_sequence":12,"selection_observed_at_nanos":24}}},"controller_shapes":{"pxmj1_phase":"receipt_durable","pxmj1_archived_active_present":false,"pxmj2_phase":"R","pxmj2_snapshot_sequence":4,"pxdm_record_sequences":[1,2,3],"pxdo_receipt_sequence":1,"pxmj2_full_frame_shapes":{"admitted":"A_h1_none","committed":"C_h2_C1_pin2","applying":"P_h3_C1_P2_pin2","active_ready":"R_h4_C1_P2_R3_PXDO1_pin2_pxmt_active_ready","failed_pre_c":"F_h2_F1_PXDO1_pin0_starting_generation","uncertain_pre_c":"U_h2_U1_PXDO1_pin0_starting_generation","failed_post_c":"F_h3_C1_F2_PXDO1_pin2_pxmt_absent_digest0_pre_send_proven_no_effect","uncertain_post_c":"U_h3_C1_U2_PXDO1_pin2","failed_post_p":"F_h4_C1_P2_F3_PXDO1_pin2_pxmt_no_effect_rejected","uncertain_post_p":"U_h4_C1_P2_U3_PXDO1_pin2_pxmt_uncertain","failed_quarantined_post_p":"F_h4_C1_P2_F3_PXDO1_pin2_pxmt_quarantined","uncertain_post_p_no_pxmt":"U_h4_C1_P2_U3_PXDO1_pin2_pxmt_absent_digest0_post_send_protocol_unknown"},"runtime_terminal_mapping":{"active_ready":"R_pxmt_present_terminal_digest_nonzero","no_effect_rejected":"F_pxmt_present_terminal_digest_nonzero","quarantined":"F_pxmt_present_terminal_digest_nonzero","uncertain":"U_pxmt_present_terminal_digest_nonzero","empty_exact_zero":"invalid_no_archive","pre_send_proven_no_effect":"F_pxmt_absent_terminal_digest_zero","post_send_protocol_unknown":"U_pxmt_absent_terminal_digest_zero","quarantined_after_agent_intent":"F_pxmt_present_terminal_digest_nonzero_focused_controller_archive"},"failed_prefix":"pre_commit","uncertain_prefix":"pre_commit"},"runtime_state":{"owner_target_fingerprint_hex":"5555555555555555555555555555555555555555555555555555555555555555","transition_projection_digest_hex":"6666666666666666666666666666666666666666666666666666666666666666","snapshot_sequence":12,"activation_snapshot_sequences":{"exact_zero":9,"model_start_intent":10,"agent_start_intent":11,"active_ready":12},"runtime_host_epoch":9,"fabric_generation_high_water":7,"model_generation_high_water":8,"agent_generation_high_water":9,"legacy_revision_high_water":4,"artifact_revision_high_water":1,"activate_pending_clock_generation":3,"activate_pending_admitted_at_nanos":100,"activate_pending_deadline_nanos":6000183,"retire_pending_clock_generation":3,"retire_pending_admitted_at_nanos":100,"retire_pending_deadline_nanos":9009000178,"retire_snapshot_sequences":{"agent_retire_intent":13,"model_retire_intent":14,"fabric_stop_intent":15,"exact_zero":16},"include_writer_fence":true,"include_revision_high_water":true,"include_one_tenure_replay":true,"include_one_request_replay":true,"include_one_temporal_replay":true,"initial_exact_zero_shape":"phase1_exact_zero_initial_active_pending_terminal_absent_census0_complete","pending_activate_shape":"phase2_tag1_active_absent_terminal_absent_generations_7_8_absent","pending_agent_start_shape":"phase3_tag1_active_absent_terminal_absent_generations_7_8_9","primary_shape":"phase4_active_ready_with_matching_terminal","pending_retire_shape":"phase5_tag2_active_present_matching_terminal_generations_7_8_9","pending_model_retire_shape":"phase6_tag2_active_present_matching_terminal_generations_7_8_9","pending_fabric_stop_shape":"phase7_tag2_active_present_matching_terminal_generations_7_8_9","retired_exact_zero_shape":"phase1_exact_zero_after_tag2_retire_active_pending_absent_census0_complete_terminal_retained_generations_7_8_9","failure_snapshot_sequences":{"uncertain_after_model_intent":11,"quarantined_after_model_intent":11,"quarantined_after_agent_intent":12},"uncertain_after_model_intent_shape":"phase9_tag1_active_absent_pending_retained_generations_7_8_absent_archive_uncertain_census0_incomplete_all_ready_false","quarantined_after_model_intent_shape":"phase10_tag1_active_absent_pending_retained_generations_7_8_absent_archive_quarantined_census0_complete_fabric_ready_true","quarantined_after_agent_intent_shape":"phase10_tag1_active_absent_pending_retained_generations_7_8_9_archive_quarantined_census0_incomplete_fabric_ready_true"},"text":{"pxop_presence_lines":["flags=0,length=416","flags=1,length=656","flags=2,length=720","flags=3,length=960","flags=6,length=960","flags=7,length=1200"],"capacity_lines":["stable-components=1241344+64*(206+64)+540,result=1259164","transaction-components=1259164+1241344+270,result=2500778","snapshot-frame=1241344,result=accept","snapshot-frame=1241345,result=reject","objects=64,result=accept","objects=65,result=reject","operations=1024,result=accept","operations=1025,result=reject","quarantine=540,result=accept","quarantine=541,result=reject","indexed-object-bytes=270,extra-temp-bytes=270,count=once-each,result=accept","defense-ceiling=8388608,result=accept","defense-ceiling=8388609,result=reject","checked-add=u64-max-plus-one,result=reject"],"reference_labels":["artifact_object_ref","materialization_receipt_ref","deployment_receipt_ref"],"successor_digest_labels":["binding","compatibility","execution","assignment","cutover_marker","plan_content","source_plan","target_slice"]}}
```

ledger中未直接列出的digest、checksum、replay、channel与terminal-result bytes全部严格按本Program已冻结domain/codec从上述语义常量派生，expected bytes由独立实现生成后hardcode，不能调用production encoder充当oracle。三份签名分别使用`tenure_seed_hex`、`controller_seed_hex`、`runtime_seed_hex`，不得跨owner复用；PXDQ固定绑定primary PXAX-M，pre-C F/U fixture固定首record，post-P F/U fixture则严格使用ledger的PXMT variant或明确的PXMT-absent transport边界；v1 fixture使用legacy fresh triple与`px-fixture-echo1`，v2 fixture使用artifact fresh triple与`px-art-prefix-v1`。production绝不读取ledger或test seed：它继续在首次PXAR12 candidate前执行上述one-shot 64-byte CSPRNG并固定切分，不能从deployment operation id或fixture bytes派生。ledger的activate deadline严格为`100 + 1000000 + 2000000 + 3000000 + 23 + 29 + 31 = 6000183`；retire deadline严格为`100 + 4000000 + 5000000 + 37 + 41 + 4000000000 + 5000000000 = 9009000178`，二者不可共用。primary ActiveReady snapshot sequence为12，tag2 phase5/6/7与post-retire ExactZero必须依次为13/14/15/16。

当前golden的materialization、deployment、Runtime与temporal typed constants故意互不相同，但这种fixture取值不新增跨owner raw-inequality规则。PXMJ2必须有十二份完整outer frame而不是只靠nested PXDM/PXDO：基础十份为`artifact_f0_pxmj_v2_{admitted,committed,applying,active_ready,failed_pre_c,uncertain_pre_c,failed_post_c,uncertain_post_c,failed_post_p,uncertain_post_p}.hex`，另两份为`artifact_f0_pxmj_v2_failed_quarantined_post_p.hex`与`artifact_f0_pxmj_v2_uncertain_post_p_no_pxmt.hex`。ledger逐项固定A header1/no records；C header2/`[C1]`；P header3/`[C1,P2]`；R header4/`[C1,P2,R3]`+PXDO1；pre-C F/U header2/`[F1]`或`[U1]`+PXDO1、Plan/PXTE/PXAR absent、revision/pin0，并保留ledger nonzero Starting generation（zero-generation另作focused case）；post-C F/U header3/`[C1,F2|U2]`+PXDO1、Plan/PXTE/PXAR present、pin2，其中`failed_post_c`是pre-send proven-no-effect、PXMT absent/digest zero的shared F。四份post-P terminal都固定header4、`[C1,P2,F3|U3]`+PXDO1、pin2：基础`failed_post_p`逐byte携带NoEffectRejected PXMT，基础`uncertain_post_p`携带Uncertain PXMT，quarantined fixture携带Quarantined PXMT，三者PXMT与terminal digest nonzero；`uncertain_post_p_no_pxmt`表示post-send/partial/protocol-unknown U，PXMT absent且terminal digest zero。若P durable后仍能证明send前零Runtime effect，post-P no-PXMT F只由focused case冻结，不再增第二shared oracle。active-ready frame的admission/Receipt high-water均1、revision1、exact PlanContent/PXTE11/PXAR12/PXMT，且每份post-C PXDM/PXDO committed pin精确为2。两份v1 fixture由当前predecessor encoder首次冻结，之后必须证明v1 encoder逐byte等于它们，而不是声称仓库此前已有hardcoded PXMJ1/PXMA1 golden。

PXMA2的每个normal F0 durable phase都用十一份完整outer frame冻结：`artifact_f0_pxma_v2_exact_zero_initial.hex`为initial phase1/snapshot sequence9；`artifact_f0_pxma_v2_pending_activate.hex`为phase2/tag1/sequence10、active absent并取ledger activation clock/at/deadline；`artifact_f0_pxma_v2_pending_agent_start.hex`为phase3/tag1/sequence11；`artifact_f0_pxma_v2_active_ready.hex`为phase4/sequence12，active与terminal archive内是同一exact PXAR12/PXMT且operation id等于Runtime ApplyOperationId。phase2允许两个互斥的sequence11 terminal分支：`artifact_f0_pxma_v2_uncertain_after_model_intent.hex`为phase9/tag1，active absent、pending逐byte保留exact PXAR12/channel、Fabric7/Model8/Agent absent与activation clock/at/deadline，census0/incomplete、全部ready/dependency false、quarantine reason absent，并archive `raw_code=32`的exact Uncertain PXMT；`artifact_f0_pxma_v2_quarantined_after_model_intent.hex`为phase10/tag1，保留同一pending facts，census0/complete、Fabric ready true而其余ready/dependency false，quarantine reason精确为由ledger model-intent semantic inputs按冻结公式派生的exact digest，并archive `raw_code=30`的exact model-intent Quarantined PXMT。phase3另有`artifact_f0_pxma_v2_quarantined_after_agent_intent.hex` sequence12 terminal分支：tag1 pending逐byte保留，Fabric7/Model8/Agent9 present，census0/incomplete、Fabric ready true而其余ready/dependency false，quarantine reason精确为由ledger agent-intent semantic inputs按冻结公式派生的exact digest，archive `artifact_f0_pxmt_artifact_quarantined_after_agent_intent_v1.hex`的`raw_code=31` exact PXMT。三个failure分支都保留writer/revision/replay/high-water且不能推进ActiveReady。`artifact_f0_pxma_v2_pending_retire_current.hex`为phase5/tag2/sequence13；`artifact_f0_pxma_v2_pending_model_retire.hex`为phase6/sequence14；`artifact_f0_pxma_v2_pending_fabric_stop.hex`为phase7/sequence15；`artifact_f0_pxma_v2_exact_zero_after_retire.hex`为post-retire phase1/sequence16、active/pending absent、census0/complete=true/all-ready-false并保留generation high-water、fence/replay与原ActiveReady terminal archive。phase5/6/7逐byte保留tag2 request/channel/generation与ledger retire clock/at/deadline。negative matrix分别mutation tag、phase、active presence、request byte、response channel、每个generation、clock/at/deadline、model/agent quarantine reason、terminal PXMT与9..16 sequence；尤其phase9 Agent-present、phase10 model/agent prefix互换，以及raw code、context presence/reason与cleanup discriminator互换后重算digest、重签PXMT并重算outer checksum，都必须因phase-specific expected derivation不符而strict reject。tag3/phase8在v2 F0必须strict reject，tag2不接受新signed request或PXTE EmptyDeactivate，v1 tag2仍只按原PXAR9 DeactivateStack语义解码。
...
- PXMT-v12 correlation不能使用v9 request、assignment或slice通过；真实canonical PXAR9/PXAR12 positives、6118/6119/6630/6631 synthetic predecoder边界、PXMJ/PXMA predecessor与上述全部v2 phase outer fixtures必须证明v9/v12不能重新封装或跨版通过，且synthetic length accept绝不冒充signed-frame positive。operation-correlation negatives分别交换deployment/materialization/Runtime/temporal typed role、漂移PXAR Runtime id或PXMT/PXMA terminal id、漂移PXDM/PXDO runtime request digest，并证明C durable后重开绝不重新抽取fresh triple；它们验证role/correlation而不是仅以跨namespace raw equality拒绝。五份signed PXMT与十二份PXMJ2完整frame必须双向冻结ActiveReady→R、NoEffectRejected→F、两个Quarantined prefix→F、Uncertain→U、shared pre-send F/PXMT-absent与post-send U/PXMT-absent边界；对artifact activation构造correctly re-signed/correlated `EmptyExactZero`必须strict reject且不得archive，trusted post-send invalid semantic只能走PXMT-absent U。focused Controller case还必须直接消费agent-intent Quarantined PXMT，逐bytearchive其exact PXMT/Agent9与PXDM/PXDO terminal digest，生成F successor并在checksum、exact reopen与canonical re-encode后保持一致，不能只断言enum或digest nonzero。PXMJ2 mutation/reopen negatives逐项交换header 1/2/3/4与PXDM/PXDO committed pin 0/2，证明A无record、pre-C terminal只能0、post-C全链只能2且R header只能4。PXMA2 reopen精确保留v12 ActiveReady、tag1/2 pending、phase9/10 matching terminal、phase5/6/7与post-retire ExactZero；tag2进入9/10、phase9 Agent-present、两个Quarantined prefix互换、tag3/phase8、missing pending/terminal、版本混合或correlation缺失一律strict reject且绝不回退D0a。PXMJ2 marker存在时lower predecessor与D0a入口不能独立重开desired authority；
- Controller store-instance producer harness以两个fresh workspace的两次独立injected draw取得不同nonzero bytes；A publication前crash不赋予candidate identity且strict reopen证明absent后允许下一次重抽，A durable后的每个crash/reopen只复用exact instance。分别漂移PXMJ2 header、PXDK、PXDM、PXDO与`pxdor1`中的instance，或令它们互相mixed/cross-workspace，必须使整条owner/query chain strict reject；ledger `d0`只能由test injection进入fixture，production source/dependency guard证明零读取、零fallback。
- standalone Controller-store crash harness必须覆盖initial staging的lock/PXMJ file fsync、staging directory fsync/reopen、whole-directory rename、state-root fsync与final root/lock/PXMJ exact reopen前后，以及A/C/P/R/F/U每种permitted fixed-next的create/write/fsync/reopen、replace、top-root fsync与final reopen前后；same/cross/partial next、final+staging、extra entry、checksum/identity/head drift逐项分类。concurrency harness固定resident-lifetime lifecycle→每phase top→legacy→successor的唯一取得顺序；successor strict重验cutover marker与active predecessor后允许existing lower API显式早释legacy，top successor durable/exact reopen后再drop successor→top，任一failure按当时actual held set逆序释放，拒绝任一lower→top或重取已drop lock。P exact reopen后证明transport期间零top/legacy/successor lock而resident lifecycle lock仍持有，PXMT返回后再按同序重验。query分别覆盖final-present top-only、final-absent bootstrap gate、same-op staging/next、两个`WouldBlock`、零lower/Runtime/IPC/fsync/cleanup，且与C封存的self-contained PXMJ2投影一致；pre-C F/U逐项区分zero lower/Runtime effect、exact-attributed unknown effect与owner/identity/attribution failure。blocked/failing stdout前必须复制owned projection并drop全部CLI/transaction handles，且不能重锁或改变durable bytes。
...
其 `state` enum 严格为 `admitted|committed|applying|active_ready|failed|uncertain`；F0没有supersede state。
...
| materialization query / `admitted` | true | false | exact input 32hex | `admitted` | O | — | `[]` | 0 |
| materialization query / `materializing`：无matching PXAV时为absent child或唯一归因的empty/fixed-temp-subset/single-final/full-pair child；有唯一matching indexed PXAV时覆盖M object-terminal与先存E prefix，均为exact stable pair且零额外child/temp/remnant | true | false | exact input 32hex | `materializing` | O | — | `[]` | 0 |
| materialization query / PXAX-present terminal | 与同 state 上述值相同 | false | exact input 32hex | 对应 exact state | 按同 state | 按同 state | 按同 state | 按同 state |
...
| 任一 materialization 命令 / pre-effect或read-only strict OWNER，且零mutation已证明（包括初见partial/unknown/cross-request staging/next、final+staging、extra entry或root/lock/snapshot strict failure） | false | false | exact input 32hex | — | — | — | `PXLC-ARTIFACT-OWNER` 恰一项 | 1 |
| materialize / 本invocation已完成initial whole-directory rename + state-root fsync + final root/lock/snapshot/objects exact reopen，或已rename、owner-root fsync并exact reopen一份canonical successor；随后以strict read-only检查发现OWNER且零additional effect已证明 | false | true | exact input 32hex | — | — | — | `PXLC-ARTIFACT-OWNER` 恰一项 | 1 |
| materialize / child/temp/write/fsync/remnant的effect set或durability有任何不明，包括已证明较早successor后的additional effect | false | — | exact input 32hex | — | — | — | `PXLC-ARTIFACT-OWNER` 恰一项 | 1 |
| materialize / bit0为1时new/different operation admission（包括命中已有indexed PXAV的E候选） | false | false | exact input 32hex | — | — | — | `PXLC-ARTIFACT-CAPACITY` 恰一项 | 2 |
...
OWNER的`changed`只度量本invocation可证明的owner mutation，不代表命令成功：pre-effect/readonly且零mutation为false；initial canonical whole-directory publication与state-root fsync/final root-lock-snapshot-objects exact reopen，或一份canonical successor的rename + owner-root fsync + exact reopen，已完整证明，且随后只由strict read-only检查发现OWNER并证明零additional effect时为true；child/temp/write/fsync/remnant的effect set或durability有任何不明时为null，即使较早publication已证明。三种OWNER都保留已strict parse的input operation id且固定`state`/O/MR为null；initial/snapshot replace后的old/new publication ambiguous不属于三者，固定为UNCERTAIN/null。
...
| deploy / `committed` | true | true 或 replay/readback时 false | exact id / `committed` / PF | O / MR | — | nonzero decimal / exact `"2"` | — | — / — | — | `[]` / 0 |
| deploy / `applying` | true | true 或 replay/readback时 false | exact id / `applying` / PF | O / MR | 32hex | nonzero decimal / exact `"2"` | — | 64hex / — | — | `[]` / 0 |
| deploy / `active_ready` | true | true 或 terminal replay时 false | exact id / `active_ready` / PF | O / MR | 32hex | nonzero decimal / exact `"2"` | DR | 64hex / 64hex | `active_ready` | `[]` / 0 |
...
| deployment operation query / same-operation strict-valid A staging或canonical next | false | false | exact input id / `uncertain` / candidate中strict verified PF否则 — | candidate中strict verified O / MR否则 — | candidate中V或 — | candidate中V或 — / V或 — | candidate中V或 — | candidate中V或 — / V或 — | `uncertain` | `PXLC-DEPLOY-UNCERTAIN` / 1 |
| deployment operation query / final-absent bootstrap lifecycle lock或strict-valid top lock `WouldBlock` | false | false | exact input id / `uncertain` / — | — / — | — | — / — | — | — / — | `uncertain` | `PXLC-DEPLOY-UNCERTAIN` / 1 |
| deploy / 首次mutating resident lifecycle owner lock `WouldBlock` | false | false | exact input id / `uncertain` / gate前strict verified PF否则 — | gate前strict verified O / MR；否则 — / — | — | — / — | — | — / — | `uncertain` | `PXLC-DEPLOY-UNCERTAIN` / 1 |
| deploy / 任一phase top/legacy/successor lock `WouldBlock` | false | 本invocation累计零mutation为false；此前完整publication且其后零effect/ambiguity为true；任一先前或当前effect/durability不明为 — | exact input id / `uncertain` / PF | O / MR | verified V或 — | verified V或 — / V或 — | verified V或 — | verified V或 — / V或 — | `uncertain` | `PXLC-DEPLOY-UNCERTAIN` / 1 |
| deploy / attribution与owner identity可信、仅lower/Runtime effect outcome或durability不明，top可靠settle exact U | false | 本调用U commit为true、terminal replay为false | exact input id / `uncertain` / PF | O / MR | verified V或 — | verified V或 — / V或 — | DR | verified V或 — / V或 — | `uncertain` | `PXLC-DEPLOY-UNCERTAIN` / 1 |
| deploy / U successor publication ambiguous或owner record不足 | false | — | exact input id / `uncertain` / PF | O / MR | verified V或 — | verified V或 — / V或 — | verified V或 — | verified V或 — / V或 — | `uncertain` | `PXLC-DEPLOY-UNCERTAIN` / 1 |
| deploy / valid attributable same-operation canonical successor的old/new publication或durability仍ambiguous | false | — | exact input id / `uncertain` / PXDQ已验证时PF否则 — | 已验证项原样保留否则 — / — | V 或 — | V 或 — / V 或 — | — | V 或 — / — | `uncertain` | `PXLC-DEPLOY-UNCERTAIN` / 1 |
| deployment operation query / checksum、mixed/cross-operation、contradictory committed head或其他strict OWNER | false | false | exact input id / — / — | — / — | — | — / — | — | — / — | — | `PXLC-DEPLOY-OWNER` / 1 |
| deploy / pre-effect strict OWNER且零mutation已证明，包括checksum、mixed/cross-operation或contradictory committed head | false | false | exact input id / — / — | — / — | — | — / — | — | — / — | — | `PXLC-DEPLOY-OWNER` / 1 |
| deploy / initial whole-directory或canonical successor已rename、parent fsync、final root/lock/PXMJ exact reopen，随后strict read-only发现OWNER且零additional effect已证明 | false | true | exact input id / — / — | — / — | — | — / — | — | — / — | — | `PXLC-DEPLOY-OWNER` / 1 |
| deploy / top/lower owner identity、operation attribution、codec/head integrity失真且已可能effect，或无法可靠记录failure | false | — | exact input id / — / — | — / — | — | — / — | — | — / — | — | `PXLC-DEPLOY-OWNER` / 1 |
...
D0b 两条命令的 `mode` 始终是 string `"local"`、`current_health_checked` 始终是 boolean `false`。terminal DR 对 `R|F|U` 都非 null；失败状态的 `V` 只允许来自 terminal PXDM/PXDO 的 nonzero verified prefix，不能从文件存在、进程状态或更后阶段的猜测补值。`PXLC-DEPLOY-OWNER`时除已strict parse并回显的input operation id外，`state`、profile、O、MR、generation、revision、controller snapshot、DR、两个Runtime digest与terminal outcome全部null；query的`changed`固定false。deploy的OWNER changed三态与ArtifactStore同义：pre-effect且零mutation证明为false；initial whole-directory或canonical successor完整rename/fsync/exact reopen后，仅由strict read-only检查发现OWNER且零additional effect证明为true；top/lower owner identity、operation attribution、codec/head integrity失真且已可能effect，或无法可靠记录其failure时为null。仅lower/Runtime effect outcome或durability不明、identity与attribution仍可信时不得归OWNER：top可可靠settle exact U successor则返回DEPLOY-UNCERTAIN，本调用terminal commit为true、纯replay为false；U publication ambiguous或record不足为UNCERTAIN/null。valid attributable same-operation的其他old/new publication/durability ambiguous同样是UNCERTAIN/null。F0遇到任何既有desired仍必须pre-effect replace-required；R0必须另建显式wire/store successor，不能在本schema增加superseded。

public `controller_snapshot_sequence`只投影latest verified PXDM/PXDO的committed snapshot pin，不投影PXMJ2 header owner snapshot sequence：A/admitted以及pre-C F/U中wire pin为0时JSON固定null；C/P/R与任一post-C F/U固定canonical decimal string `"2"`。PXMJ2 header内部仍是A=1、C=2、P=3、R=4或后继F/U的N+1，任何mapper把header 1/3/4公开为controller snapshot都必须失败。ledger的`controller_committed_snapshot_sequence = 2`是public oracle，`pxmj2_snapshot_sequence = 4`只验证R frame header；hardcoded JSON/golden与source guard必须同时锁定两者不可互换。
...
| `PXLC-DEPLOY-EXTERNAL-JSON-OUTPUT` | `external deployment JSON output could not be written` | 1 |

JSON-output code不得跨D0a/D0b复用：既有D0a继续逐byte使用`PXLC-DEPLOY-JSON-OUTPUT`/`compiled local deployment JSON output failed`，D0b两条external命令只使用上表的新code/message。hardcoded guard fixture `artifact_f0_d0a_json_output_diagnostic_v1.json`与`artifact_f0_d0b_json_output_diagnostic_v1.json`分别固定为下列单行canonical compact diagnostic object加一个LF；serializer/taxonomy test必须证明两个code/message双向不互换，且D0b total order中不存在旧D0a code：

```json
{"code":"PXLC-DEPLOY-JSON-OUTPUT","message":"compiled local deployment JSON output failed"}
```

```json
{"code":"PXLC-DEPLOY-EXTERNAL-JSON-OUTPUT","message":"external deployment JSON output could not be written"}
```
...
| D0b `deploy` | `PXLC-DEPLOY-EXTERNAL-GRAMMAR` → `PXLC-ARG-NON-UTF8` → `PXLC-PLATFORM-UNSUPPORTED` → `PXLC-EXECUTION-IDENTITY` → `PXLC-CONFIG-PATH-INVALID` → `PXLC-CONFIG-FILE-READ` → `PXLC-CONFIG-FILE-TOO-LARGE` → `PXLC-CONFIG-DOCUMENT-INVALID` → `PXLC-CONFIG-SCHEMA-UNSUPPORTED` → `PXLC-STATE-ROOT-TOO-LONG` → `PXLC-STATE-ROOT-INVALID` → `PXLC-FABRIC-LISTEN-INVALID` → `PXLC-MODEL-MISSING` → `PXLC-CONFIG-PROVIDER-INVALID` → `PXLC-MODEL-INVALID` → `PXLC-CONFIG-PROVIDER-UNKNOWN` → `PXLC-LIFECYCLE-CONFIGURATION` → `PXLC-DEPLOY-ARTIFACT` → `PXLC-DEPLOY-MATERIALIZATION-RECEIPT` → `A0_APPLICATION_ADMISSION_REQUIRED` → `PXLC-DEPLOY-CONFLICT` → `PXLC-DEPLOY-REPLACE-REQUIRED` → `PXLC-DEPLOY-CAPACITY` → `PXLC-DEPLOY-FAILED` → `PXLC-DEPLOY-UNCERTAIN` → `PXLC-DEPLOY-OWNER` → `PXLC-DEPLOY-EXTERNAL-JSON-OUTPUT` |
| `deployment operation query` | `PXLC-DEPLOYMENT-QUERY-GRAMMAR` → `PXLC-ARG-NON-UTF8` → `PXLC-PLATFORM-UNSUPPORTED` → `PXLC-EXECUTION-IDENTITY` → `PXLC-CONFIG-PATH-INVALID` → `PXLC-CONFIG-FILE-READ` → `PXLC-CONFIG-FILE-TOO-LARGE` → `PXLC-CONFIG-DOCUMENT-INVALID` → `PXLC-CONFIG-SCHEMA-UNSUPPORTED` → `PXLC-STATE-ROOT-TOO-LONG` → `PXLC-STATE-ROOT-INVALID` → `PXLC-FABRIC-LISTEN-INVALID` → `PXLC-MODEL-MISSING` → `PXLC-CONFIG-PROVIDER-INVALID` → `PXLC-MODEL-INVALID` → `PXLC-CONFIG-PROVIDER-UNKNOWN` → `PXLC-LIFECYCLE-CONFIGURATION` → `PXLC-DEPLOY-NOT-FOUND` → `PXLC-DEPLOY-FAILED` → `PXLC-DEPLOY-UNCERTAIN` → `PXLC-DEPLOY-OWNER` → `PXLC-DEPLOY-EXTERNAL-JSON-OUTPUT` |
...
- mutating command 和 query 对 `admitted|materializing|materialized|already_materialized|committed|applying|active_ready` 的有效 owner-correlated响应使用 `ok = true`、exit 0；`failed|uncertain` 使用 `ok = false`、exit 1 并保留已验证的 operation/ref/Receipt/terminal 字段，不得将其清空后伪装成“未发生”。query 找不到 operation、owner/receipt/correlation/timeout/I/O/output failure同样 exit 1；只有已识别 namespace 的 grammar、path/config/ref/profile/compatibility 或 pre-effect conflict 为 exit 2。stdout 可写时严格为一个 compact JSON object 加一个 LF，stderr 为空；不能完整交付 JSON 时仍 exit 1。
...
ADR-0011 Accepted + authorization receipt（decision complete）
  └─> Artifact F0 single admission candidate（r606 joint candidate已验证，待merge/release）
...
| A1 external Artifact build/inspect/materialize | Validated only inside r606 joint candidate（无独立admission；待merge/release） | ADR-0011 Accepted + receipt；与D0b同一candidate条件已满足；若触发A0还需其最小ADR Accepted | 候选内构建、inspect并物化`developer-local-echo-prefix-v1`供D0b真实消费；不单独声称用户可用 | r606同ref六条合同中的前四条 grammar/JSON/reference/profile golden；reproducible pair、strict inspect、tamper/compatibility、crash/durability/idempotency/Receipt 与 noexec/no-install/no-Graph；A1 15/15 real-binary system evidence，但这些证据仍不产生独立A1 admission |
...
| D0b external-artifact local deploy | Validated joint candidate（r606；已登记，待merge/release） | 同candidate内A1 internal evidence、D0a r363与ADR-0011 gate均已满足 | 候选在独立fresh workspace消费exact pair/owner Receipt，由Controller/Runtime到达ActiveReady并由TUI/Agent观察external prefix；main用户surface仍等待merge | r606 exact grammar/JSON/fresh-only；pair进入Plan/Slice；Controller/Runtime/TUI真实消费与Receipt correlation；D0b 1/1 real-binary scenario、607/2完整pytest、workspace Rust/governance/ProcessDomain/doctest；replay/conflict/output-loss/Uncertain/tamper-race/no-second-writer/no-double-active同ref证据 |
| R0 replace/restart | Planned/Blocked（r606已验证，等待joint candidate merge；未实现） | D0b r606 validated candidate + merge gate | 显式替换新 Artifact；或在同 Artifact/state 上安全停止后重启，不透明自动 retry | forward DeploymentRevision、generation fencing、joined stop、SIGKILL/owner loss、stale generation、same-state restart、partial failure 与 no-double-active 证据 |
...
Artifact F0不改变上述M2a normal-path grammar、9-key schema或generic lifecycle projection，但增加上文唯一cutover start gate与A→Starting并发边界：plain `up`只可跟随same-config external Starting/Running owner，不可在matching marker下启动compiled fallback或未授权restart；would-be start按configuration→replace/reconcile优先级失败。`status`/`down`在control unavailable且NeverStarted/Stopped/pre-record候选时必须对既有owner lock做一次nonblocking held-check，A→record窗口固定unknown/unavailable而不能误报no-op；external `down`只驱动tag2 graceful retire→ExactZero并保留historical PXMJ2-R。除这组Artifact cutover carve-out外，既有M2a字节与正常行为不变。
...
A1 的 Accepted decision gate、Program contract与候选实现已完成，但delivery状态只随r606 joint candidate升级为`Validated inside joint candidate`：Artifact crate/store、前四CLI、governance rows、A1 system harness与D0b真实Controller/Runtime/TUI consumer在同一ref齐备并通过门禁；A1仍不能独立进入main/merge/admission或声称用户可用。r606尚待正常merge/release。实现严格消费上文 profile/reference/grammar/JSON：build 从 exact source 产生 byte-identical payload/canonical manifest，inspect 严格只读且零 mutation，ArtifactStore 以 durable admission→pair publication→exact reopen→terminal record/Receipt 的唯一顺序物化完整 ref。任何 tamper、truncate、oversize、unknown version/field、pair swap、ABI/target/entrypoint mismatch 必须在 store mutation 前失败关闭。

A1 internal claim-to-evidence 必须在最终包含D0b的同一 immutable candidate ref完整覆盖；任何较早内部commit的结果只能是开发证据，不能单独升级状态：

- 单一canonical semantic ledger是全部wire/state golden的唯一测试常量源；hardcoded Rust/Python goldens逐byte覆盖206-byte PXAM、72-byte PXAK、PXAQ/PXAA/PXMU/PXAV/PXAW/PXAX、672/912/976/1104/1216/1408/1456/1648/2304/2608/2848-byte PXAZ frame及其672/912/976/1330/1216/1634/1456/1874/2530/2834/3074-byte logical accounting、PXAY/PXOP、五份PXAW-only query JSON、capacity vectors、text object/Receipt refs、全部SHA-256 domain、pair swap与全部frozen JSON key order；两种语言独立strict decode同一hardcoded expected bytes，不调用production encoder生成期望值；其中PXMU-absent F与PXMU-present F各有PXAW-only/PXAX full snapshot，后者固定flags3/7、bit0=0、quarantine=0、零child/remnant并strict关联PXAW materializing digest；2304-byte second-operation E materializing prefix以first-operation M receipt与indexed pair为唯一history，不得由producer动态拼装。同输入reproducible build byte/digest一致，payload/string/limit/reserved/flag/order/checksum/short/trailing与1241344/1241345-byte边界各有正反例；
- build 不执行 payload；hardcoded build-output-path domain与105-byte fixed sibling name由Rust/Python独立重算。crash/concurrency harness覆盖immediate-parent strict pin、staging `mkdirat`、parent fsync、same-parent empty reopen、两file write/fsync/reopen、staging fsync/reopen、rename前identity重验、`RENAME_NOREPLACE`、parent fsync/reopen与same-parent final/pair reopen前后；existing empty/partial/full staging全部UNCERTAIN/false且零恢复/cleanup，same-invocation staging或rename durability不明为UNCERTAIN/null。exact-final replay必须在本调用fsync两file、final directory与pinned immediate parent，并从same reopened parent exact reopen final/pair全链后才返回false；任一barrier失败或publication ambiguity为UNCERTAIN/null。conflicting final固定PATH/false，final永不暴露partial pair且同一output最多一个fixed sibling。source guard拒绝内部`.next`/random temp/lock、recursive mkdir、absolute-path reopen与inspect触碰staging。inspect 前后 workspace/store/lifecycle/domain digest 不变；tamper、truncate、oversize、unknown、payload/manifest pair swap、相同 payload/不同 manifest 与 unsupported ABI/target/entrypoint 都在 pre-effect拒绝；
- 首次materialize在全部config/pair/A0/path preflight后才建立non-authoritative state-root scaffolding：immediate parent必须既有且strict pin，leaf只可`mkdirat` no-overwrite 0700→parent fsync→same-parent exact reopen/revalidate，绝不recursive mkdir；mkdir后、parent-fsync前后、leaf reopen前后与config-path swap的crash/race逐项证明ambiguous scaffold为ARTIFACT-UNCERTAIN/null、identity/path失真按PATH/OWNER边界失败，且在任何instance draw/store authority前可重建或补齐exact empty leaf。随后才以含首个PXAQ/PXAA的snapshot-sequence-1 staging directory做唯一atomic `RENAME_NOREPLACE` authority publication；valid attributable same-request staging/canonical same-operation next在active无terminal时精确UNCERTAIN，different/partial/unknown、final+staging、extra entry与root/lock/snapshot strict failure精确OWNER且owner fields为null，final-without-valid-snapshot绝不按zero store处理。same operation + same request只replay/advance同一snapshot record，same operation + different request/ref pre-effect conflict；current/PXAZ/PXAQ config与D0b PXDQ/Receipt-chain config的两类错误按冻结total order稳定区分；
- admission、flags0→1 PXMU、pinned-objects parent下no-overwrite child mkdir、child/parent fsync与exact reopen、两份pair temp write/sync、各final no-overwrite publish、final child fsync、same-pinned-parent pair reopen、PXAV、PXAW、PXAX每个独立snapshot successor与Receipt output逐点crash。fault points至少位于mkdir后/parent fsync前、parent fsync后/child reopen前、每份temp与final、child fsync前后、same-parent reopen前后及PXAV前后；每份next精确N+1并在replace前后复核root/lock/final。restart先收口next再锁存PXMU recovery-start：snapshot已有matching PXAV时，未被更早terminal PXAW引用且唯一归属最后incomplete本operation的prefix必须M，已被更早terminal operation引用的prefix才是E；无PXAV且在“mkdir后、parent fsync前”或后续窗口重见existing child/final pair的positive recovery，必须证明先fsync pinned parent、close/exact reopen parent+child并复核identity/entry set，之后才refsync pair、fsync child、same-parent reopen并以同一successor加PXAV/HW后终结M。attributed partial/remnant的positive U recovery必须证明PXAV absent、所有regular remnants、child、parent均fsync，且close parent后从pinned root exact reopen parent、再从该parent exact reopen child/entries的全链成立，才可发布bit0-U；每个root/parent/child/file fsync与reopen fault都返回OWNER且snapshot中无新U/PXAV。OWNER changed harness必须固定三类：pre-effect snapshot corruption且零mutation证明为OWNER/false；valid next已rename、owner-root fsync、exact reopen为canonical successor后，仅由strict read-only enumeration发现unexpected extra entry且零additional effect证明为OWNER/true；pinned `objects/` parent fsync失败或child/temp/write/remnant effect/durability不明均为OWNER/null，即使较早successor已证明。另以snapshot replace后old/new publication不可判定证明UNCERTAIN/null，绝不混入OWNER。F必须证明零pair effect/remnant；U必须PXAV absent且在PXAX前后都保留exact quarantine/block。golden与fault harness拒绝交换M/E，证明零目录推理、零新operation、零overwrite/delete/path fallback；
- materializing query matrix除无matching PXAV的absent/empty/temp/single-final/full-pair五种形态外，还必须覆盖本operation 1104-byte object-terminal M prefix与更早terminal operation的indexed-PXAV E prefix；两者都返回同一materializing/false envelope且零mutation，不能由query选择M/E。对两类matching-PXAV state分别加入extra child/temp/remnant与pair-chain drift，全部精确OWNER。
- U/OWNER classification fault matrix必须分开验证：exact operation attribution与owner identity均成立、但pair effect/durability outcome不确定的attributed remnant只有完成全部file/child/parent fsync及pinned-root reopen链后才可发布PXAW-U/PXAX-U；逐项漂移root/parent/child/file identity、operation attribution或cross-request bytes必须返回OWNER，且snapshot中无新PXAW-U/PXAX-U。不能用同一个generic failure case覆盖两类。
- private absolute root与0700目录、固定exact-three stable entries、0600 regular single-link lock/pair、owner/mode、178-byte suffix/3917-byte state-root边界、path traversal、symlink/hardlink、nlink/inode replacement、short write与no-overwrite；strict-valid lock的WouldBlock/EWOULDBLOCK对materialize/query都输出exact UNCERTAIN，其他lock syscall为IO，metadata/identity/layout invalid为OWNER，且try-lock先于本invocation任何effect。shared/exclusive lock无retry，Drop explicit unlock，fork/spawn/exec前drop全部clone且child防御性unlock/close。initial staging在mkdir、lock create/fsync/parent reopen、objects mkdir/fsync/parent reopen各barrier产生的exact empty、lock-only、lock+empty-objects三种non-authoritative prefix都用query concurrency/crash fixture冻结：empty线性化NotFound，后两种shared lock free为NotFound、WouldBlock为UNCERTAIN，snapshot任一bytepartial或缺lock/objects为OWNER。query对virgin/operation-absent分别NotFound且零创建，M/E/PXMU-absent F/PXMU-present F/U五种PXAW-only输入逐行返回exact JSON；另以PXMU durable而无PXAW/matching PXAV的empty child、fixed temp subset、single final、full pair四种materializing crash fixture证明exact materializing成功envelope与全store逐byte零mutation，unknown/cross-operation/second child、fixed subset外entry及indexed-PXAV命中时的额外child逐项OWNER。所有query都绝不fsync/cleanup/recover/补PXAX；
...
- PXAA operation sequence与PXAV object sequence各从1逐次+1并和各自snapshot successor的high-water/count同commit，replay沿用，overflow/ambiguous publication失败关闭。精确验证64 objects、1024 operations、PXAV-absent last-U在PXAX前后quarantine归因、zero-byte attributed entry、540/541-byte边界、1241152-byte body与1241344-byte snapshot；两份canonical blocked fault-state分别为PXAW-U/PXAX-absent与matching PXAX-U/present，均只有exact-attributed unindexed child/remnant且无PXAV。由该blocked store发起任一different operation——包括其PXAK恰好命中另一个已indexed object的E候选——都必须在PXAQ/PXAA前返回exact `PXLC-ARTIFACT-CAPACITY` envelope且snapshot、object directories、counts/high-waters逐byte不变，同时同一quarantine-U exact replay/PXAX completion与read-only query仍可用。1259164只声明componentwise conservative logical regular-file accounting upper bound，不声明64个indexed objects、最大snapshot与540-byte unindexed quarantine可由同一canonical history同时达到，2500778只声明componentwise conservative transaction upper，两者都排除directory/zero lock/filesystem blocks。capacity text fixture只喂pure/injected checked calculator的isolated component caps与overflow，不制造canonical state；calculator证明8388608接受、8388609或overflow拒绝，同时证明count caps令public v1不能自然耗尽8-MiB defense ceiling，不用padding、假history或超额quarantine造边界；
...
- 仅与本合同相关的 `tests/fixtures/**/*artifact*`、`tests/system/test_a1_local_artifact_cli.py`、`tests/system/test_d0b_external_artifact_deploy_cli.py`、`tests/contract/test_artifact_f0_d0b_owner_frames.py`、以及只为保持既有Deployment public API exact-set guard而同步新增Artifact facade集合的`tests/governance/{test_s7_pure_compile_boundary.py,test_s7_tenure_authority_boundary.py}`、`.github/workflows/{ci.yml,macos-cli-artifact.yml}`；只有A1 producer、D0b Controller/Runtime/TUI consumer、六条命令与两个system harness在同一candidate存在时才于该candidate更新`governance.toml`，任何A1-only阶段不预登记且不可merge。两份既有S7 guard只能继续做完整集合相等检查并为新增Artifact集合增加精确owner/compatibility断言，不得放宽为子集、通配或跳过。
...
D0b 的 intended-public contract、owner seam、实现、治理登记与joint evidence已在r606同一immutable ref收口，状态为`Validated joint candidate`，不是等待一个已独立admitted A1的blocked step，也尚不是main已发布surface。r363 D0a、ADR-0011 Accepted/receipt及A1-internal机制前置均已满足；下一执行动作是正常review/merge该joint candidate，随后才能进入R0。D0b 只在 workspace B 的 fresh lifecycle/deployment state中消费已 terminal materialized 的完整 ref 与 owner Receipt，且 D0b 必须是第一次 lifecycle mutation；不得先调用 D0a `run_up()`、读取 workspace A、保留旧 desired、用 compiled-in fallback或让 ArtifactStore选择 active object。

Controller 必须把完整 ref、materialization Receipt commitment、profile/ABI/target/entrypoint写入 exact ArtifactExecutionBinding、PlanContent v2 与 PXTE11/PXAR12 Slice，再由 PXMJ2 唯一top-level desired authority独占 commit/revision/deployment operation/Receipt。Runtime 对 authenticated apply 与 exact Slice 做 read-only pre-effect reopen/reverify；CLI返回`state = "active_ready"`的唯一owner gate是Runtime exact PXMT `ActiveReady` terminal与Controller的完整correlation，不等待TUI/Agent observation，也不从其响应反推owner state。Materialized、Committed、Applying、Runtime terminal与Ready互不推导；Inspection仍独占当前 health。

r606验证锚点是branch `build/ubuntu-validation-r606`的commit `e175f2a75c299f67e89ef1b7308f0263d363148e`与GitHub Ubuntu run [`31555989066`](https://github.com/jsmy-CTH/ParaEGOX/actions/runs/31555989066)。`governance-and-tests` job `93988405445` 对同一exact binary先独立执行A1 15/15与D0b 1/1，再通过Ruff、完整governance、全量pytest 607 passed/2 skipped及Rust→Python ProcessDomain；两项skip仅是既有G2/T1 harness要求root后降权而GitHub runner非root，不属于Artifact F0场景。`rust-core` job `93988405483`通过format、locked metadata、workspace all-targets check、Clippy `-D warnings`、workspace tests、Rust worker ProcessDomain与全部doctest。该证据把A1/D0b joint candidate升级为Validated，但不声称已merge/release，不实现R0/D1，不解冻remote，也不完成整个Program。

D0b claim-to-evidence 必须在同一 immutable candidate ref 完整覆盖：

- 六条中后两条 exact grammar/JSON、D0a 五 token不变、help与strict dispatch；malformed deploy的三个D0b reserved option精确归external grammar而其余旧D0a negatives逐 byte不变。D0a cutover hardcodes原14-key replace-required/lifecycle/configuration JSON，up保持原9-key，chat保持stdout-empty/exact-stderr；以different-config valid final、A staging与canonical next、invalid/mixed evidence及A staging/final publication race证明configuration→replace/reconcile优先级和所有返回都发生在`run_up`、lifecycle record/socket与lower effect前，commitment不可信时绝不raw-offset猜config。status/down A→record held-lock barrier与plain-up external-winner follow/post-down拒绝同样逐byte冻结；其余D0a/M2a/chat bytes不变。D0a/D0b JSON-output hardcoded diagnostic vectors与source guard分别钉旧`PXLC-DEPLOY-JSON-OUTPUT`和新`PXLC-DEPLOY-EXTERNAL-JSON-OUTPUT`，双向交换失败。workspace B fresh initial成功，workspace A 既有 D0a desired精确返回replace-required，fresh workspace不得先出现D0a lifecycle mutation；
- complete object ref/materialization Receipt/profile/ABI/target/entrypoint逐项进入 exact 192-byte binding、PlanContent v2、PXTE11/PXAR12与target Slice digest；D0b admission只接受M/E的exact PXAX→PXAW→PXMU→PXAV→PXAA→PXAQ→PXAM/payload链，并证明current/PXAZ/PXAQ/PXDQ四份config commitment相等。current-vs-root mutation稳定命中`PXLC-LIFECYCLE-CONFIGURATION`，PXDQ-vs-receipt mutation稳定命中`PXLC-DEPLOY-MATERIALIZATION-RECEIPT`；hardcoded successor/predecessor golden和cross-version negatives同时通过。DeploymentController 独占revision/deployment operation/Receipt；PXDQ/PXDK/PXDM/PXDO deployment id全链不变，materialization id只由ArtifactStore Receipt链解释；source/dependency guard证明Controller不读payload/store目录；
- Controller store-instance producer以两个fresh workspace独立injected 32-byte draw、A前/A后crash与wrong-instance chain证明：A未durable不赋予identity且可在strict reopen证明absent后重抽，A durable后PXMJ2/PXDK/PXDM/PXDO/query/`pxdor1`只复用exact bytes；ledger d0常量仅test injection，production零读取/零fallback。
- standalone top-store evidence逐fault覆盖A staging与A/C/P/R/F/U fixed-next的file/directory fsync、rename/replace、root fsync及exact reopen，same/cross/partial next、final+staging与root/lock/PXMJ metadata/identity/codec drift；十二份PXMJ2完整outer goldens锁A/C/P/R、pre-C/post-C F/U及四种post-P F/U的header/body/PXMT presence/chain/checksum与public committed pin，五份signed PXMT独立锁ActiveReady/NoEffectRejected/Uncertain/model-intent Quarantined/agent-intent Quarantined，PXMA2十一份完整outer goldens锁initial1、activation2/3/4、failure9/model-Q10/agent-Q10与retire5/6/7/post1序列。resident lifecycle lock从A前贯穿joined shutdown，每phase只按lifecycle→top→legacy→successor取得，successor strict重验后允许existing lower API早释legacy，再在top successor durable/reopen后drop successor→top；所有failure按实际held set逆序释放，P后transport期间无Controller/lower lock且PXMT返回后同序重开。首次mutating owner-lock concurrency barrier必须证明恰一winner、contender立即返回hardcoded UNCERTAIN/false且零top/lower/Starting effect，winner结束后只有新调用same-id replay/query可继续；不得把plain-up follow语义带入D0b。query final-present只读self-contained top，final-absent才走bootstrap shared gate；两条路径、staging/next与contended lock都证明零lower/Runtime/IPC/fsync/cleanup。top-only query只做canonical/checksum/nested digest/correlation，不声称独立Ed25519验证；signature drift通过结构链失败为OWNER，mutating reopen/replay在ordered lower scope用可信keys重验两签名。lower open/init自身cleanup/write/unlink/fsync纳入F/U/OWNER effect分类；pre-C F只在effect与durability全确定时成立，exact attribution+identity下effect outcome不明才U，identity/attribution失败则OWNER。所有return path在JSON前drop调用侧handles，blocked/failing stdout不重锁或改写owner bytes。
- Deployment OWNER envelope/fault matrix逐项覆盖checksum、mixed-version/cross-operation、contradictory committed head、valid canonical next及publication前后：query strict OWNER固定changed=false且除strict input id外全部projection null；deploy pre-effect corruption为OWNER/false；initial whole-directory rename + state-root fsync + final root/lock/PXMJ exact reopen，或canonical successor完整settle/reopen后，由pure read-only extra-entry检查发现OWNER且零additional effect时为true；任何后续write/fsync/Runtime effect或durability不明为OWNER/null。valid attributable same-operation old/new publication ambiguous单独固定为DEPLOY-UNCERTAIN/null。每例都复核exact diagnostic/exit与零未声明effect。
...
- owner Receipt refs、Controller revision、Runtime apply request digest与PXMT terminal digest全链相关；一次64-byte CSPRNG draw产生Runtime ApplyOperationId/TemporalConstraintId/auth nonce并在C首次durable，PXMT/PXMA2 terminal id等于PXAR Runtime id，PXDM/PXDO只以exact request digest桥接且不把deployment/materialization id当Runtime id。single semantic ledger、fixed injected constants与role-swap/correlation-drift negatives证明typed separation及C后不重抽，跨namespace raw coincidence本身不触发额外reject；五份signed PXMT、四种shared post-P Controller frame、agent-intent Quarantined focused Controller archive、shared pre-send F与EmptyExactZero negative逐byte证明唯一outcome mapping和present/nonzero-digest或absent/zero-digest边界；独立digest oracle从ledger semantic facts按三条既有v1 domain/字段顺序派生resource/raw/quarantine actual bytes并hardcode进五份PXMT与十一份PXMA，code/context/cleanup交换后即使重算、重签与重写checksum仍按phase-specific expected derivation拒绝，production encoder不得自证。CLI/file/process/ACK/log/TUI response不能合成Ready，`current_health_checked = false`。同candidate system test只在owner-correlatedActiveReady与external Running generation成立后attach TUI/Agent并逐byte观察`prefix || prompt`，该observation只是admission evidence，不进入PXMJ/PXMT/CLI state；
- fault/race覆盖 admitted→committed→applying→active_ready、failed/uncertain、Controller commit后Runtime失败、supervisor crash、down/output fault、stale generation与并发 follower；PXMJ2 marker/cutover、lower predecessor重开、PXMA2 phase5/6/7→ExactZero与v1/v2 cross-reject证明任何时刻无第二writer/desired head、无active pointer、无两个live generation/double-active，也不破坏workspace A的D0a/M3/M5基线。
...
M5a 是 additive public grammar。新 `paraegox tui` 只附着已经 Running/ready 的 generation，并为这个新入口增加 private pinned-handoff child mode。既有 `paraegox chat` 继续保留“由当前前台 composition 启动、向 Python direct typed clients交付现有两个 private bootstrap path、并在 child 退出后 joined shutdown”的 public 与内部边界；它的normal absent-marker path不改handoff、grammar、presentation或joined lifecycle，也不以新Program静默supersede ADR-0009。唯一carve-out是Artifact F0 cutover：foreground chat必须由resident lifecycle gate串行并按configuration→replace/reconcile优先级拒绝external marker，绝不启动旧compiled stack；这新增了已冻结的exact exit/stderr，但不改变正常chat语法或Python direct-client边界。两个入口最终都仍是 Python direct typed clients；M5a 只是让 attach 路径在 locator→child-open race中携带完整 pins，而不是引入 Rust `ConsoleBridge` 的后继。
...
9. 在 workspace B 以 D0b exact deploy作为第一次 lifecycle/deployment mutation，消费同一完整 object ref/materialization Receipt；DeploymentController提交Plan/Slice/revision，Runtime pre-effect reopen复验并以exact PXMT `ActiveReady`与Controller correlation到达owner terminal。随后同candidate system test附着该external Running generation，由真实TUI/Agent逐byte观察`prefix || prompt`作为非owner admission evidence。Materialized/Committed/Applying/Ready各自由owner evidence证明；output loss以原operation query，任何缺证据保持Failed/Uncertain。workspace A的D0a/M3/M5 generation、revision和owner state必须保持不变。
...
- ADR-0011 的 Accepted 状态与有效 authorization receipt 已完成决策 gate；本 Program 冻结的A1/D0b intended-public contract及A1 mechanism/evidence→fresh D0b real consumer顺序已由r606同一immutable ref实现并验证。Artifact crate/store与前四CLI仍没有独立A1 admission；A1 producer、D0b Controller/Runtime/TUI consumer、六条命令、两个system harness与governance rows只以joint candidate共同成立。r606仍待正常merge/release，R0继续等待该gate；其验证不等于R0/D1实现、remote解冻或Program完成。

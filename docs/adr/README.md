...
- [ADR-0011 — 本地不可变 Artifact 物化与 Deployment 选择边界](ADR-0011-local-immutable-artifact-materialization-and-deployment-selection.md) — Proposed；compiled-in deterministic D0a不依赖本决策，未来单external-Artifact只候选准入绑定payload/manifest digest pair的完整`ArtifactObjectRefV1`与ArtifactStore immutable materialization，DeploymentController与RuntimeHost继续分别独占desired selection和live generation，Runtime只读重开复验而不成为第二物化写者；不建立Application/Installation、active pointer、卸载/GC或通用Graph/workflow owner。

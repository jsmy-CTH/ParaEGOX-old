...
- [ADR-0011 — 本地不可变 Artifact 物化与 Deployment 选择边界](ADR-0011-local-immutable-artifact-materialization-and-deployment-selection.md) — Accepted；compiled-in deterministic D0a不依赖本决策，首个真实external-Artifact方向只准入被Runtime/Agent/TUI链实际消费的bounded non-executable deterministic model-data，以payload/manifest digest pair的完整`ArtifactObjectRefV1`进入ArtifactStore immutable materialization；ArtifactStore、DeploymentController与RuntimeHost继续分别独占物化写入、desired selection和live generation，Runtime只读重开复验；不建立Application/Installation、active pointer、卸载/GC、任意代码加载或通用Graph/workflow owner，具体实现仍须Program另行准入。
